...
var _ = g.Describe("[OTP][sig-network-edge] Network_Edge Component_ALBO", func() {
	...
	g.It("[Level0] Author:hongli-ROSA-OSD_CCS-ConnectedOnly-High-51189-Install aws-load-balancer-operator and controller [Serial]", func() {
		var (
	...
	g.It("Author:hongli-ROSA-OSD_CCS-ConnectedOnly-Medium-51191-Provision ALB by creating an ingress [Serial]", func() {
		...
		// skip if worker nodes in local-zone or wavalength-zone
		worker, _ := oc.AsAdmin().WithoutNamespace().Run("get").Args("nodes", "--selector=machine.openshift.io/zone-type").Output()
		if !strings.Contains(worker, "No resources found") {
			g.Skip("Skipping since auto-discover subnets cannot work with nodes in local-zone or wavelength-zone")
		}
