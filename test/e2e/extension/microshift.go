...
import (
	...
	"github.com/openshift/router/test/e2e/extension/testdata"
)
...
var _ = g.Describe("[OTP][sig-network-edge] Network_Edge", func() {
	...
	g.It("Author:shudili-MicroShiftOnly-NonPreRelease-Longduration-High-73203-configuring listening IP addresses and listening Ports [Disruptive]", func() {
		...
		exutil.By(`1. Create a namespace for testing, then debug node and get all valid host interfaces and invalid host ips`)
		defer oc.DeleteSpecifiedNamespaceAsAdmin(e2eTestNamespace)
		oc.CreateSpecifiedNamespaceAsAdmin(e2eTestNamespace)
		exutil.SetNamespacePrivileged(oc, e2eTestNamespace)
		...
		exutil.By("2. Debug node to backup the config.yaml, and restore it before the test finishes running")
		defer restoreConfigYaml(oc, e2eTestNamespace, caseID, nodeName)
		backupConfigYaml(oc, e2eTestNamespace, caseID, nodeName)

		exutil.By(`3. Debug node and configure ingress with the desired listening IP addresses and listening Ports`)
...
ingress:
    ...
    ports:
		...
		exutil.By("4. Wait the check router-default service is updated and its load balancer ip is as same as configured in default.yaml")
		regExp := "^" + randHostIP + "$"
		...
		exutil.By("5. Check service router-default's http port is changed to 10080 and its https port is changed to 10443")
		...
		exutil.By("6. Deploy a client pod, a backend pod and its services resources")
		createResourceFromFile(oc, e2eTestNamespace, clientPod)
		ensurePodWithLabelReady(oc, e2eTestNamespace, clientPodLabel)
		createResourceFromFile(oc, e2eTestNamespace, testPodSvc)
		ensurePodWithLabelReady(oc, e2eTestNamespace, "name=web-server-deploy")

		exutil.By("7. Create a HTTP/Edge/Passthrough/REEN route")
		...
		exutil.By("8. Curl the routes with destination to the the custom load balancer ip and http/https ports")
		...
		exutil.By("9. Curl the http route")
		// config firewall for ipv6 load balancer
		...
		exutil.By("10. Curl the Edge route")
		...
		exutil.By("11. Curl the Passthrough route")
		...
		exutil.By("12. Curl the REEN route")
	...
	g.It("Author:shudili-MicroShiftOnly-NonPreRelease-Longduration-High-73209-Add enable/disable option for default router [Disruptive]", func() {
		var (
			caseID           = "73209"
			e2eTestNamespace = "e2e-ne-" + caseID + "-" + getRandomString()
		)

		exutil.By("1. Create a namespace for testing, then debug node and get the valid host ips")
		defer oc.DeleteSpecifiedNamespaceAsAdmin(e2eTestNamespace)
		oc.CreateSpecifiedNamespaceAsAdmin(e2eTestNamespace)
		exutil.SetNamespacePrivileged(oc, e2eTestNamespace)
		...
		exutil.By("3. Debug node to disable the default router by setting ingress status to Removed")
		ingressConfig := fmt.Sprintf(`
ingress:
    status: Removed`)
		...
		exutil.By("4. Check the openshift-ingress namespace will be deleted")
		...
		exutil.By(`5. Debug node to enable the default router by setting ingress status to Managed`)
		ingressConfig = fmt.Sprintf(`
ingress:
    status: Managed`)
		...
		exutil.By("6. Check router-default load balancer is enabled")
	...
	g.It("Author:shudili-MicroShiftOnly-NonPreRelease-Longduration-Medium-73621-Disable/Enable namespace ownership support for router [Disruptive]", func() {
		...
		exutil.By(`3. Debug node to disable namespace ownership support by setting namespaceOwnership to Strict in the config.yaml file"`)
		ingressConfig := fmt.Sprintf(`
ingress:
    routeAdmissionPolicy:
		...
		exutil.By("4. Check the Env ROUTER_DISABLE_NAMESPACE_OWNERSHIP_CHECK of deployment/default-router, which should be false")
		...
		exutil.By("5. Create a server pod and the services in one ns")
		createResourceFromFile(oc, e2eTestNamespace1, testPodSvc)
		ensurePodWithLabelReady(oc, e2eTestNamespace1, "name="+srvrcInfo)

		exutil.By("6. Create a route with path " + path1 + " in the first ns, which should be admitted")
		...
		exutil.By("7. Create a server pod and the services in the other ns")
		createResourceFromFile(oc, e2eTestNamespace2, testPodSvc)
		ensurePodWithLabelReady(oc, e2eTestNamespace2, "name="+srvrcInfo)

		exutil.By("8. Create a route with path " + path2 + " in the second ns, which should NOT be admitted")
		...
		exutil.By("9. Check the two routes with same hostname but with different path for the second time, the first one is adimitted, while the second one isn't")
		...
		exutil.By("10. Confirm the second route is shown as HostAlreadyClaimed")
		...
		exutil.By("11. Debug node to enable namespace ownership support by setting namespaceOwnership to InterNamespaceAllowed in the config.yaml file")
		ingressConfig = fmt.Sprintf(`
ingress:
    routeAdmissionPolicy:
		...
		exutil.By("12. Check the two route with same hostname but with different path, both of them should be adimitted")
		ensureRouteIsAdmittedByIngressController(oc, e2eTestNamespace1, "route-http", "default")
		ensureRouteIsAdmittedByIngressController(oc, e2eTestNamespace2, "route-http", "default")

		exutil.By("13. Confirm no route is shown as HostAlreadyClaimed")
	...
	g.It("Author:shudili-MicroShiftOnly-High-77349-introduce ingress controller customization with microshift config.yaml [Disruptive]", func() {
		...
		exutil.By(`5.0: Debug node to configure the ingress in the config.yaml`)
		ingressConfig := fmt.Sprintf(`
ingress:
    ...
    tuningOptions:
		...
		exutil.By("6.0 Check the router-default deployment that all updated ENVs of tested parameters are as expected")
		for _, routerEntry := range allParas {
			jsonPath := fmt.Sprintf(`{.spec.template.spec.containers[0].env[?(@.name=="%s")].value}`, routerEntry[0])
			envValue := getByJsonPath(oc, "openshift-ingress", "deployment/router-default", jsonPath)
		...
		exutil.By("7.0 Check the haproxy.config that all updated vaules of tested parameters are set as expected")
		routerpod = getOneRouterPodNameByIC(oc, "default")
		for _, routerEntry := range allParas {
...
`, baseDomain)
		...
		exutil.By(`5.0: Debug node and configure the default certification for the ingress`)
		ingressConfig := fmt.Sprintf(`
ingress:
    certificateSecret: "%s"`, customCert)
		...
		exutil.By("7.0: Create an edge route for the testing")
		...
		exutil.By("8.0: Check the router-default deployment that the volume of default certificate is updated to the custom")
		...
		exutil.By("9.0: Check the customed default certification in a router pod")
		routerpod := getOneRouterPodNameByIC(oc, "default")
		...
		exutil.By("10.0: Curl the edge route with the user certification, issued by MS-default-CA")
		...
		exutil.By("11.0: Curl the edge route again without any certification")
	...
	g.It("Author:shudili-MicroShiftOnly-High-80510-supporting Old tlsSecurityProfile for the ingress controller [Disruptive]", func() {
		...
		exutil.By("4.0: Debug node and configure the Old tls profile for the ingress")
		ingressConfig := fmt.Sprintf(`
ingress:
    tlsSecurityProfile:
        old: {}
		...
		// OCP-80513 - [MicroShift] supporting Intermediate tlsSecurityProfile for the ingress controller
		exutil.By("6.0: Debug node and Configure the Intermidiate tls profile for the ingress")
		ingressConfig = fmt.Sprintf(`
ingress:
    tlsSecurityProfile:
        intermediate: {}
	...
	g.It("Author:shudili-MicroShiftOnly-High-80514-supporting Modern tlsSecurityProfile for the ingress controller [Disruptive]", func() {
		...
		exutil.By("3.0: Debug node and configure the Modern tls profile for the ingress")
		ingressConfig := fmt.Sprintf(`
ingress:
    tlsSecurityProfile:
        modern: {}
		...
		exutil.By("6.0: Debug node and configure the Custom tls profile for the ingress")
		ingressConfig = fmt.Sprintf(`
ingress:
    tlsSecurityProfile:
        custom:
...
ingress:
	...
	// includes OCP-81996 and OCP-84261
	// OCP-81996 - capture and log http cookies with specific prefixes via httpCaptureCookies option
	// OCP-84261 - capture and log http cookies by the "httpCaptureCookies" option for Edge route and REEN route
	// author: shudili@redhat.com
	g.It("Author:shudili-MicroShiftOnly-NonPreRelease-High-81996-capture and log http cookies with specific prefixes via httpCaptureCookies option [Disruptive]", func() {
		var (
			...
			edgeRouteHost       = "route-edge" + caseID + "." + baseDomain
			reenRouteHost       = "route-reen" + caseID + "." + baseDomain
		)

		exutil.By("1.0: Prepare a namespace for the testing")
		defer oc.DeleteSpecifiedNamespaceAsAdmin(e2eTestNamespace)
		oc.CreateSpecifiedNamespaceAsAdmin(e2eTestNamespace)
		exutil.SetNamespacePrivileged(oc, e2eTestNamespace)
		...
		exutil.By(`3.0: Debug node and configure "capture and log http cookies with specific prefixes via httpCaptureCookies option"`)
		ingressConfig := fmt.Sprintf(`
ingress:
    accessLogging:
        httpCaptureCookies:
		...
		exutil.By("4.0: Create a client pod, a deployment and the services")
		createResourceFromFile(oc, e2eTestNamespace, clientPod)
		ensurePodWithLabelReady(oc, e2eTestNamespace, clientPodLabel)
		createResourceFromFile(oc, e2eTestNamespace, testPodSvc)
		ensurePodWithLabelReady(oc, e2eTestNamespace, "name="+srvrcInfo)

		exutil.By("5.0: Create a http/edge/reen route for the testing")
		...
		exutil.By("6.0: Check httpCaptureCookies configuration in haproxy")
		routerpod := getOneNewRouterPodFromRollingUpdate(oc, "default")
		// ensureHaproxyBlockConfigContains(oc, routerpod, "defaults", []string{"capture cookie foo len 100"})

		exutil.By("7.0: Curl the http route with cookie fo=nobar, expect to get 200 OK")
		podIP := getPodv4Address(oc, routerpod, "openshift-ingress")
		toDst := routehost + ":80:" + podIP
		...
		exutil.By("8.0: Curl the http route with cookie foo=bar, expect to get 200 OK")
		...
		exutil.By("8.1: Curl the http route with cookie foo22=bar22, expect to get 200 OK")
		...
		exutil.By("8.2: Curl the edge route with cookie foo=barforedge, expect to get 200 OK")
		toDst = edgeRouteHost + ":443:" + podIP
		curlCmd = []string{"-n", e2eTestNamespace, clientPodName, "--", "curl", "https://" + edgeRouteHost + "/index.html", "-skI", "-b", "foo=barforedge", "--resolve", toDst, "--connect-timeout", "10"}
		repeatCmdOnClient(oc, curlCmd, "200 OK", 90, 6)

		exutil.By("8.3: Curl the reen route with cookie foo=barforreen, expect to get 200 OK")
		toDst = reenRouteHost + ":443:" + podIP
		curlCmd = []string{"-n", e2eTestNamespace, clientPodName, "--", "curl", "https://" + reenRouteHost + "/index.html", "-skI", "-b", "foo=barforreen", "--resolve", toDst, "--connect-timeout", "10"}
		repeatCmdOnClient(oc, curlCmd, "200 OK", 90, 6)

		exutil.By("9.0: Check the router logs, which should contain both the cookie foo=bar and the url")
		logs := waitRouterLogsAppear(oc, routerpod, "foo=bar")
		o.Expect(logs).To(o.ContainSubstring("index.html"))

		exutil.By("10.0: Check the router logs, which should NOT contain both the cookie fo=nobar")
		containerName := getByJsonPath(oc, "openshift-ingress", "pod/"+routerpod, "{.spec.containers[*].name}")
		logs, err := oc.AsAdmin().WithoutNamespace().Run("logs").Args("-n", "openshift-ingress", "-c", strings.Split(containerName, " ")[1], routerpod).Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(logs).NotTo(o.ContainSubstring("fo=nobar"))

		exutil.By("11.0: Check the router logs, which should contain both the cookie foo22=bar22 and the url")
		logs = waitRouterLogsAppear(oc, routerpod, "foo22=bar22")
		o.Expect(logs).To(o.ContainSubstring("index.html"))

		// check logs for edge route of OCP-84261
		exutil.By("12.0: Check the router logs, which should contain both the cookie foo=barforedge and the url for the edge route")
		logs = waitRouterLogsAppear(oc, routerpod, "foo=barforedge")
		o.Expect(logs).To(o.ContainSubstring("index.html"))

		// check logs for reen route of OCP-84261
		exutil.By("13.0: Check the router logs, which should contain both the cookie foo=barforreen and the url for the reen route")
		logs = waitRouterLogsAppear(oc, routerpod, "foo=barforreen")
		o.Expect(logs).To(o.ContainSubstring("index.html"))
	})
    ...
    accessLogging:
		...
		exutil.By("5.0: Create a http route for the testing")
		createRoute(oc, e2eTestNamespace, "http", "route-http", unsecSvcName, []string{"--hostname=" + routehost})
		ensureRouteIsAdmittedByIngressController(oc, e2eTestNamespace, "route-http", "default")
		...
		exutil.By("7.0: Curl the http route with cookie fooor=nobar, expect to get 200 OK")
		podIP := getPodv4Address(oc, routerpod, "openshift-ingress")
		toDst := routehost + ":80:" + podIP
		...
		exutil.By("10.0: Check the router logs, which should NOT contain the cookie fooor=nobar")
		containerName := getByJsonPath(oc, "openshift-ingress", "pod/"+routerpod, "{.spec.containers[*].name}")
		logs, err := oc.AsAdmin().WithoutNamespace().Run("logs").Args("-n", "openshift-ingress", "-c", strings.Split(containerName, " ")[1], routerpod).Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		exutil.By("11.0: debug node and configure maxLength for the httpCaptureCookies")
		ingressConfig = fmt.Sprintf(`ingress:
    accessLogging:
        httpCaptureCookies:
            - matchType: Exact
		...
		exutil.By("12.0: Curl the http route with cookie foo=bar89abdef, expect to get 200 OK")
		routerpod = getOneNewRouterPodFromRollingUpdate(oc, "default")
		podIP = getPodv4Address(oc, routerpod, "openshift-ingress")
		toDst = routehost + ":80:" + podIP
		...
		exutil.By("13.0: Check the router logs, which should contain the cookie foo=bar89a, NOT foo=bar89abdef")
		logs = waitRouterLogsAppear(oc, routerpod, "foo=bar89a")
		o.Expect(logs).NotTo(o.ContainSubstring("foo=bar89ab"))
	})

	// includes OCP-82000, OCP-82002 and OCP-84259
	// OCP-82000 - capture and log specific http Request header via httpCaptureHeaders option
	// OCP-82002 - capture and log specific http Response headers via httpCaptureHeaders option
	// OCP-84259 - capture and log specific http header via "httpCaptureHeaders" option for Edge route and REEN route
	// author: shudili@redhat.com
...
ingress:
    accessLogging:
		...
		exutil.By("6.0: Check httpCaptureHeaders configuration in haproxy")
		routerpod := getOneNewRouterPodFromRollingUpdate(oc, "default")
		frontendCfg := getBlockConfig(oc, routerpod, "frontend fe_sni")
		o.Expect(frontendCfg).To(o.ContainSubstring("capture request header Host len 120"))
		// check configuration for OCP-82002
		o.Expect(frontendCfg).To(o.ContainSubstring("capture response header Server len 120"))

		exutil.By("7.0: Curl the http route, expect to get 200 OK")
		podIP := getPodv4Address(oc, routerpod, "openshift-ingress")
		toDst := routehost + ":80:" + podIP
		curlCmd := []string{"-n", e2eTestNamespace, clientPodName, "--", "curl", "http://" + routehost + "/index.html", "-sI", "--resolve", toDst, "--connect-timeout", "10"}
		repeatCmdOnClient(oc, curlCmd, "200 OK", 90, 6)

		exutil.By("7.1: Curl the edge route, expect to get 200 OK")
		toDst = edgeRouteHost + ":443:" + podIP
		curlCmd = []string{"-n", e2eTestNamespace, clientPodName, "--", "curl", "https://" + edgeRouteHost + "/index.html", "-skI", "--resolve", toDst, "--connect-timeout", "10"}
		repeatCmdOnClient(oc, curlCmd, "200 OK", 90, 6)

		exutil.By("7.2: Curl the reen route, expect to get 200 OK")
		toDst = reenRouteHost + ":443:" + podIP
		curlCmd = []string{"-n", e2eTestNamespace, clientPodName, "--", "curl", "https://" + reenRouteHost + "/index.html", "-skI", "--resolve", toDst, "--connect-timeout", "10"}
		repeatCmdOnClient(oc, curlCmd, "200 OK", 90, 6)

		// check logs for OCP-82002
		exutil.By("8.0: Check the router logs, which should contain both the request host and the response server")
		logs := waitRouterLogsAppear(oc, routerpod, routehost)
		o.Expect(logs).To(o.MatchRegexp(server + "/" + "[0-9\\.]+"))

		// check logs for edge route of OCP-84259
		exutil.By("9.0: Check the router logs, which should contain both the request host of the edge route and the response server")
		logs = waitRouterLogsAppear(oc, routerpod, edgeRouteHost)
		o.Expect(logs).To(o.MatchRegexp(server + "/" + "[0-9\\.]+"))

		// check logs for reen route of OCP-84259
		exutil.By("10.0: Check the router logs, which should contain both the request host of the reen route and the response server")
		logs = waitRouterLogsAppear(oc, routerpod, reenRouteHost)
		o.Expect(logs).To(o.MatchRegexp(server + "/" + "[0-9\\.]+"))
	})
	...
	g.It("Author:shudili-MicroShiftOnly-High-82003-The httpCaptureHeaders option strictly adheres to the maxlength parameter [Disruptive]", func() {
		var (
			...
			e2eTestNamespace    = "e2e-ne-" + caseID + "-" + getRandomString()
			baseDomain          = "apps.example.com"
			routehostPrefix     = "route-unsec" + caseID
			routehost           = routehostPrefix + "." + baseDomain
		)
		...
		exutil.By("3.0: Debug node and configure a small maxLength for request/response httpCaptureHeaders")
		ingressConfig := fmt.Sprintf(`
ingress:
    accessLogging:
        httpCaptureHeaders:
		...
		exutil.By("6.0: Curl the http route, expect to get 200 OK")
		routerpod := getOneNewRouterPodFromRollingUpdate(oc, "default")
		podIP := getPodv4Address(oc, routerpod, "openshift-ingress")
		toDst := routehost + ":80:" + podIP
		curlCmd := []string{"-n", e2eTestNamespace, clientPodName, "--", "curl", "http://" + routehost + "/index.html", "-sI", "--resolve", toDst, "--connect-timeout", "10"}
		...
		exutil.By("7.0: Check the router logs, which should contain the request host header and response server header with values not exceeding the maxLength")
		logs := waitRouterLogsAppear(oc, routerpod, routehostPrefix)
		o.Expect(logs).NotTo(o.ContainSubstring(routehost))
	...
	g.It("Author:shudili-MicroShiftOnly-High-82004-custom http 503 and 404 error pages for ingress route [Disruptive]", func() {
		var (
			...
			baseDomain          = "apps.example.com"
			routehost           = "route-unsec" + caseID + "." + baseDomain
			notExistHost        = "not-exist" + caseID + "." + baseDomain
		)

		exutil.By("1.0  Create a configmap with the customized error pages in openshift-ingress namespace")
		_, err := oc.AsAdmin().WithoutNamespace().Run("create").Args("configmap", configmapName, "--from-file="+cmFile1, "--from-file="+cmFile2, "-n", "openshift-ingress").Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		defer oc.AsAdmin().WithoutNamespace().Run("delete").Args("configmap", configmapName, "-n", "openshift-ingress").Output()

		exutil.By("2.0: Prepare a namespace for the following testing")
		defer oc.DeleteSpecifiedNamespaceAsAdmin(e2eTestNamespace)
		oc.CreateSpecifiedNamespaceAsAdmin(e2eTestNamespace)
		exutil.SetNamespacePrivileged(oc, e2eTestNamespace)

		exutil.By("3.0: Debug node to backup the config.yaml, and restore it before the test finishes running")
		nodeName := getByJsonPath(oc, "default", "nodes", "{.items[0].metadata.name}")
		defer restoreConfigYaml(oc, e2eTestNamespace, caseID, nodeName)
		backupConfigYaml(oc, e2eTestNamespace, caseID, nodeName)
		actualGenerationInt := getRouterDeploymentGeneration(oc, "router-default")

		exutil.By("4.0: Debug node and configure custom ingress httpErrorCodePages")
		ingressConfig := fmt.Sprintf(`
ingress:
    httpErrorCodePages:
        name: %s
`, configmapName)
		...
		exutil.By("6.0: Create a http route for the testing")
		createRoute(oc, e2eTestNamespace, "http", "route-http", unsecSvcName, []string{"--hostname=" + routehost})
		ensureRouteIsAdmittedByIngressController(oc, e2eTestNamespace, "route-http", "default")

		exutil.By("7.0: Check customized error 503 and 404 pages in haproxy")
		routerpod := getOneNewRouterPodFromRollingUpdate(oc, "default")
		errorfileCfg := readRouterPodData(oc, routerpod, "cat /var/lib/haproxy/errorfiles/error-page-503.http", "Unavailable")
		o.Expect(errorfileCfg).Should(o.And(
			o.ContainSubstring(`HTTP/1.0 503 Service Unavailable`),
			o.ContainSubstring(`Custom:Application Unavailable`)))

		errorfileCfg = readRouterPodData(oc, routerpod, "cat /var/lib/haproxy/errorfiles/error-page-404.http", "Not Found")
		o.Expect(errorfileCfg).Should(o.And(
			o.ContainSubstring(`HTTP/1.0 404 Not Found`),
			o.ContainSubstring(`Custom:Not Found`)))

		exutil.By("8.0: Curl a not exist http route, expect to get the custom 404 error page")
		podIP := getPodv4Address(oc, routerpod, "openshift-ingress")
		toDst := notExistHost + ":80:" + podIP
		curlCmd := []string{"-n", e2eTestNamespace, clientPodName, "--", "curl", "http://" + notExistHost, "-s", "--resolve", toDst, "--connect-timeout", "10"}
		repeatCmdOnClient(oc, curlCmd, "Custom:Not Found", 60, 1)

		exutil.By("9.0: Scale down the deployment to 0, the curl a http route, expect to get the custom 503 error page")
		scaleDeploy(oc, e2eTestNamespace, srvrcInfo, 0)
		toDst = routehost + ":80:" + podIP
		curlCmd = []string{"-n", e2eTestNamespace, clientPodName, "--", "curl", "http://" + routehost, "-s", "--resolve", toDst, "--connect-timeout", "10"}
		repeatCmdOnClient(oc, curlCmd, "Custom:Application Unavailable", 60, 1)
	})
	...
	g.It("Author:shudili-MicroShiftOnly-NonPreRelease-High-82014-httpLogFormat for logging http requests [Disruptive]", func() {
		...
		exutil.By(`3.0: Debug node and configure httpLogFormat with %{+Q}r`)
		val := `%{+Q}r`
		ingressConfig := fmt.Sprintf(`
ingress:
    accessLogging: 
...
`, val)
		...
		exutil.By("5.0: Create a http route for the testing")
		createRoute(oc, e2eTestNamespace, "http", "route-http", unsecSvcName, []string{"--hostname=" + routehost})
		waitForOutputEquals(oc, e2eTestNamespace, "route/route-http", jsonPath, "True")

		exutil.By("6.0: Check httpLogFormat configuration in haproxy")
		routerpod := getOneNewRouterPodFromRollingUpdate(oc, "default")
		// OCPBUGS-60257(indentation issue of "frontend public" in haproxy)
		// formatCfg := getBlockConfig(oc, routerpod, "defaults")
		// expectedVal := fmt.Sprintf(`log-format "%s"`, val)
		// o.Expect(formatCfg).To(o.ContainSubstring(expectedVal))

		exutil.By("7.0: Curl the http route, expect to get 200 OK")
		podIP := getPodv4Address(oc, routerpod, "openshift-ingress")
		toDst := routehost + ":80:" + podIP
		curlCmd := []string{"-n", e2eTestNamespace, clientPodName, "--", "curl", "http://" + routehost + "/path/second/index.html", "-sI", "--resolve", toDst, "--connect-timeout", "10"}
		repeatCmdOnClient(oc, curlCmd, "200 OK", 90, 6)

		exutil.By("8.0: Check the router logs, which should contain the desired info including the http request url")
		logs := waitRouterLogsAppear(oc, routerpod, "/path/second/index.html")
		o.Expect(logs).To(o.MatchRegexp(`haproxy\[[0-9]+\]: "HEAD /path/second/index.html HTTP`))

		exutil.By(`9.0: Debug node and configure httpLogFormat with %ci:%cp %si:%sp %HU %ST`)
		val = `%ci:%cp %si:%sp %HU %ST`
		ingressConfig = fmt.Sprintf(`
ingress:
    accessLogging:
        httpLogFormat: "%s"
...
`, val)
		...
		exutil.By("10.0: Check new httpLogFormat configuration in haproxy")
		routerpod = getOneNewRouterPodFromRollingUpdate(oc, "default")
		// OCPBUGS-60257(indentation issue of "frontend public" in haproxy)
		// formatCfg = getBlockConfig(oc, routerpod, "defaults")
		// expectedVal = fmt.Sprintf(`log-format "%s"`, val)
		...
		exutil.By("11.0: Curl the http route, expect to get 200 OK")
		podIP = getPodv4Address(oc, routerpod, "openshift-ingress")
		toDst = routehost + ":80:" + podIP
		curlCmd = []string{"-n", e2eTestNamespace, clientPodName, "--", "curl", "http://" + routehost + "/path/second/index.html", "-sI", "--resolve", toDst, "--connect-timeout", "10"}
		repeatCmdOnClient(oc, curlCmd, "200 OK", 90, 6)

		exutil.By("12.0: Check the router logs, which should contain the desired info including source & destination address and http status code")
		logs = waitRouterLogsAppear(oc, routerpod, "/path/second/index.html")
		o.Expect(logs).To(o.MatchRegexp(`haproxy\[[0-9]+\]: [0-9\\.a-fA-F:]+:[0-9]+ [0-9\\.a-fA-F:]+:8080 /path/second/index.html 200`))
	})
	...
	g.It("Author:shudili-MicroShiftOnly-NonPreRelease-High-82015-logging destination to a syslog server [Disruptive]", func() {
		...
		exutil.By("2.0: Create a client pod, a rsyslogd pod, a deployment and the services")
		createResourceFromFile(oc, e2eTestNamespace, clientPod)
		ensurePodWithLabelReady(oc, e2eTestNamespace, clientPodLabel)
		createResourceFromFile(oc, e2eTestNamespace, syslogPod)
		ensurePodWithLabelReady(oc, e2eTestNamespace, syslogPodLable)
		createResourceFromFile(oc, e2eTestNamespace, testPodSvc)
		ensurePodWithLabelReady(oc, e2eTestNamespace, "name="+srvrcInfo)
		...
		exutil.By(`4.0: Debug node and configure the syslog destination`)
		syslogIP := getByJsonPath(oc, e2eTestNamespace, "pods/"+syslogPodName, "{.status.podIP}")
		ingressConfig := fmt.Sprintf(`
ingress:
    accessLogging:
        destination:
...
`, syslogIP)
		...
		exutil.By("6.0: Check the syslog configuration in haproxy")
		routerpod := getOneNewRouterPodFromRollingUpdate(oc, "default")
		syslogCfg := getBlockConfig(oc, routerpod, "global")
		expectedVal := fmt.Sprintf(`log %s:514 len 1024 local1 info`, syslogIP)
		o.Expect(syslogCfg).To(o.ContainSubstring(expectedVal))
		...
		exutil.By("8.0: Check the logs of syslog pod, which should contain the haproxy log")
		logs, err := oc.AsAdmin().WithoutNamespace().Run("logs").Args("-n", e2eTestNamespace, syslogPodName, "--tail=20").Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(logs).To(o.MatchRegexp(`haproxy\[[0-9]+\]:.+/path/second/index.html`))

		exutil.By(`9.0: Debug node and configure destination syslog with facility local2`)
		ingressConfig = fmt.Sprintf(`
ingress:
    accessLogging:
        destination:
...
`, syslogIP)
		...
		exutil.By("10.0: Check the new syslog configuration in haproxy")
		routerpod = getOneNewRouterPodFromRollingUpdate(oc, "default")
		syslogCfg = getBlockConfig(oc, routerpod, "global")
		expectedVal = fmt.Sprintf(`log %s:514 len 1024 local2 info`, syslogIP)
		o.Expect(syslogCfg).To(o.ContainSubstring(expectedVal))
	})

	// includes OCP-84260 and OCP-82042
	// OCP-84260 Negative test of the logging configuration
	// OCP-82042 disable logging function
	// author: shudili@redhat.com
	g.It("Author:shudili-MicroShiftOnly-Medium-84260-Negative test of the logging configuration [Disruptive]", func() {
		var (
			caseID           = "84260"
			e2eTestNamespace = "e2e-ne-" + caseID + "-" + getRandomString()
		)
		...
		exutil.By(`3.0: Debug node and try to configure invalid maxLength -1 of the httpCaptureCookies`)
		ingressConfig := fmt.Sprintf(`ingress:
    accessLogging:
        httpCaptureCookies:
            - matchType: Exact
		...
		output := appendInvalidIngressConfigToConfigYaml(oc, e2eTestNamespace, caseID, nodeName, ingressConfig)
		o.Expect(output).To(o.ContainSubstring("Must be between 1 and 1024"))

		exutil.By(`4.0: Debug node and try to configure invalid maxLength 0 of the httpCaptureCookies`)
		ingressConfig = fmt.Sprintf(`ingress:
    accessLogging:
        httpCaptureCookies:
            - matchType: Exact
		...
		exutil.By(`5.0: Debug node and try to configure the name of the httpCaptureCookies with specail characters including space, # and so on`)
		ingressConfig = fmt.Sprintf(`ingress:
    accessLogging:
        httpCaptureCookies:
            - matchType: Exact
		...
		output = appendInvalidIngressConfigToConfigYaml(oc, e2eTestNamespace, caseID, nodeName, ingressConfig)
		exutil.By("output is :" + output)
		o.Expect(output).To(o.ContainSubstring("contains invalid characters"))

		exutil.By(`6.0: Debug node and try to configure invalid maxLength -1 of the httpCaptureHeaders`)
		ingressConfig = fmt.Sprintf(`ingress:
    accessLogging:
        httpCaptureHeaders:
            request:
		...
		output = appendInvalidIngressConfigToConfigYaml(oc, e2eTestNamespace, caseID, nodeName, ingressConfig)
		o.Expect(output).To(o.ContainSubstring("maxLength must be at least 1"))

		exutil.By(`7.0: Debug node and try to configure status with invalid Enable(not Enabled)`)
		ingressConfig = fmt.Sprintf(`ingress:
    accessLogging:
        httpCaptureHeaders:
            request:
		...
		output = appendInvalidIngressConfigToConfigYaml(oc, e2eTestNamespace, caseID, nodeName, ingressConfig)
		o.Expect(output).To(o.ContainSubstring("invalid access logging status: Enable"))

		// OCP-82042 disable logging function
		exutil.By("8.0: Debug node and configure capture and log http cookies with specific prefixes via httpCaptureCookies option, but without enable the status")
		ingressConfig = fmt.Sprintf(`
ingress:
    accessLogging:
        httpCaptureCookies:
...
`)
		...
		appendIngressToConfigYaml(oc, e2eTestNamespace, caseID, nodeName, ingressConfig)
		ensureRouterDeployGenerationIs(oc, "default", strconv.Itoa(actualGenerationInt))

		exutil.By("9.0: Check the configuration of haproxy which should not contain the logging httpCaptureCookies")
		routerpod := getOneNewRouterPodFromRollingUpdate(oc, "default")
		defaultsCfg := getBlockConfig(oc, routerpod, "defaults")
		o.Expect(defaultsCfg).NotTo(o.ContainSubstring("capture cookie foo len 100"))
	})
})
