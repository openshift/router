...
var _ = g.Describe("[OTP][sig-network-edge] Network_Edge Component_Router", func() {
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-Critical-11903-haproxy cookies based sticky session for unsecure routes", func() {
		...
		exutil.By("2.0: Prepare file for testing")
		updateFilebySedCmd(testPodSvc, "replicas: 1", "replicas: 2")

		exutil.By("3.0: Create a client pod and two server pods and the service")
		ns := oc.Namespace()
		...
		exutil.By("4.0: Create a plain HTTP route")
		...
		exutil.By("5.0: Curl the http route, make sure the second server is hit")
		...
		exutil.By("6.0: Curl the http route again, make sure the first server is hit")
		...
		exutil.By("7.0: Curl the http route with the specified cookie for 10 times, expect all are forwarded to the second server")
		...
		// OCP-11679
		exutil.By(`8.0: Disable haproxy hash based sticky session for the route by adding 'disable cookies' annotation to it`)
		setAnnotation(oc, ns, "route/"+unsecSvcName, "haproxy.router.openshift.io/disable_cookies=true")

		exutil.By("9.0: Check the disable cookies configuration in haproxy")
		...
		exutil.By("10.0: Curl the http route with the specified cookie for 10 times again, expect the requests are forwarded to the two servers")
		_, result = repeatCmdOnClient(oc, curlCmd, expectOutput, 120, 10)
