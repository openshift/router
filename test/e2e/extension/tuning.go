...
var _ = g.Describe("[OTP][sig-network-edge] Network_Edge Component_Router", func() {
	...
	g.It("Author:mjoseph-Critical-40747-The 'tune.bufsize' and 'tune.maxwrite' values can be modified by ingresscontroller tuningOptions", func() {
		...
		exutil.By("2: Check the haproxy config on the router pod for existing maxrewrite and bufsize value")
		ensureHaproxyBlockConfigContains(oc, routerpod, "global", []string{"tune.bufsize 16385", "tune.maxrewrite 4097"})

		exutil.By("3: Create a second custom ingresscontroller, and get its router name")
		baseDomain = getBaseDomain(oc)
		...
		exutil.By("4: Patch the second ingresscontroller with maxrewrite and bufsize value")
		...
		exutil.By("5: Check the haproxy config on the router pod of second ingresscontroller for the tune.bufsize buffer value")
		...
		exutil.By("6: Patch ingresscontroller with minimum values and check whether it is configurable")
		...
		exutil.By("7: Patch ingresscontroller with tune.maxrewrite buffer value")
		...
		exutil.By("8: Check the haproxy config on the router pod for the tune.maxrewrite buffer value")
		...
		exutil.By("9: Patch ingresscontroller with tune.bufsize buffer value")
		...
		exutil.By("10: check the haproxy config on the router pod for the tune.bufsize buffer value")
		newrouterpod = getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)
	...
	g.It("[Level0] Author:shudili-Critical-41110-The threadCount ingresscontroller parameter controls the nbthread option for the haproxy router", func() {
		buildPruningBaseDir := testdata.FixturePath("router")
		...
		exutil.By("1: Create a ingresscontroller with threadCount set")
		baseDomain := getBaseDomain(oc)
		ingctrl.domain = ingctrl.name + "." + baseDomain
		defer ingctrl.delete(oc)
		ingctrl.create(oc)
		...
		exutil.By("2: Check the router env to verify the default value of ROUTER_THREADS is applied")
		...
		exutil.By("3: Patch the new ingresscontroller with tuningOptions/threadCount " + threadcount)
		ingctrlResource := "ingresscontrollers/" + ingctrl.name
		patchResourceAsAdmin(oc, ingctrl.namespace, ingctrlResource, "{\"spec\": {\"tuningOptions\": {\"threadCount\": "+threadcount+"}}}")
		ensureRouterDeployGenerationIs(oc, ingctrl.name, "2")

		exutil.By("4: Check the router env to verify the PROXY variable ROUTER_THREADS with " + threadcount + " is applied")
		...
		exutil.By("5: Check the haproxy config on the router pod to ensure the nbthread is updated")
		...
		exutil.By("6: Patch the new ingresscontroller with negative(" + threadcount1 + ") value as threadCount")
		...
		exutil.By("7: Patch the new ingresscontroller with high(" + threadcount2 + ") value for threadCount")
		...
		exutil.By("8: Patch the new ingresscontroller with string(" + threadcount3 + ") value for threadCount")
	...
	g.It("[Level0] Author:aiyengar-Critical-43112-Timeout tunnel parameter for the haproxy pods an be modified with TuningOptions option in the ingresscontroller", func() {
		buildPruningBaseDir := testdata.FixturePath("router")
		customTemp := filepath.Join(buildPruningBaseDir, "ingresscontroller-np.yaml")
		var (
			ingctrl = ingressControllerDescription{
		...
		exutil.By("2: Patch ingresscontroller with a tunnel timeout option")
		ingctrlResource := "ingresscontrollers/" + ingctrl.name
		patchResourceAsAdmin(oc, ingctrl.namespace, ingctrlResource, "{\"spec\":{\"tuningOptions\" :{\"tunnelTimeout\": \"2h\"}}}")
		ensureRouterDeployGenerationIs(oc, ingctrl.name, "2")
		newrouterpod := getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)

		exutil.By("3: Verify the new tls inspect timeout value in the router pod")
		...
		exutil.By("4: Patch ingresscontroller with negative values for the tuningOptions settings and check the ingress operator config post the change")
		...
		exutil.By("5: Check the timeout option set in the haproxy pods post the changes applied")
	...
	g.It("Author:shudili-High-50662-Make ROUTER_BACKEND_CHECK_INTERVAL Configurable", func() {
		...
		exutil.By("1: Create two custom ICs for testing ROUTER_BACKEND_CHECK_INTERVAL")
		baseDomain := getBaseDomain(oc)
		...
		exutil.By("2: Check ROUTER_BACKEND_CHECK_INTERVAL env in a route pod of IC ocp50662one, which should be 20s")
		...
		exutil.By("3: Check ROUTER_BACKEND_CHECK_INTERVAL env in a route pod of IC ocp50662two, which should be 100m")
		...
		exutil.By("4: Patch tuningOptions/healthCheckInterval with max 2147483647ms to IC ocp50662one, while tuningOptions/healthCheckInterval 0s to the IC ocp50662two")
		...
		exutil.By("5: Check ROUTER_BACKEND_CHECK_INTERVAL env in a route pod of IC ocp50662one, which should be 2147483647ms")
		podname1 = getOneNewRouterPodFromRollingUpdate(oc, ingctrl1.name)
		hciSearch = readRouterPodEnv(oc, podname1, "ROUTER_BACKEND_CHECK_INTERVAL")
		o.Expect(hciSearch).To(o.ContainSubstring("ROUTER_BACKEND_CHECK_INTERVAL=2147483647ms"))

		exutil.By("6: Try to find the ROUTER_BACKEND_CHECK_INTERVAL env in a route pod which shouldn't be seen by default")
		podname2 = getOneNewRouterPodFromRollingUpdate(oc, ingctrl2.name)
		...
		exutil.By("9: Try to patch tuningOptions/healthCheckInterval -1s which is a minus value, to the ingress-controller")
		...
		exutil.By("10: Try to patch tuningOptions/healthCheckInterval abc which is a string, to the ingress-controller")
	...
	g.It("Author:shudili-High-50926-Support a Configurable ROUTER_MAX_CONNECTIONS in HAproxy", func() {
		...
		exutil.By("1: Create a custom IC with tuningOptions/maxConnections -1 specified by ingresscontroller-tuning.yaml")
		baseDomain := getBaseDomain(oc)
		ingctrl.domain = ingctrl.name + "." + baseDomain
		defer ingctrl.delete(oc)
		ingctrl.create(oc)
		...
		exutil.By("2: Check ROUTER_MAX_CONNECTIONS env under a route pod for the configured maxConnections -1,  which should be auto")
		podname := getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)
		...
		exutil.By("3: Check maxconn in haproxy.config which won't appear after configured tuningOptions/maxConnections with -1")
		...
		exutil.By("4: Patch tuningOptions/maxConnections with 2000 to IC ocp50926")
		patchResourceAsAdmin(oc, ingctrl.namespace, ingctrlResource, "{\"spec\": {\"tuningOptions\": {\"maxConnections\": 2000}}}")
		ensureRouterDeployGenerationIs(oc, ingctrl.name, "2")

		exutil.By("5: Check ROUTER_MAX_CONNECTIONS env under a router pod of IC ocp50926, which should be 2000")
		podname = getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)
		maxConnSearch = readRouterPodEnv(oc, podname, "ROUTER_MAX_CONNECTIONS")
		o.Expect(maxConnSearch).To(o.ContainSubstring("ROUTER_MAX_CONNECTIONS=2000"))

		exutil.By("6: Check maxconn in haproxy.config under a router pod of IC ocp50926, which should be 2000")
		ensureHaproxyBlockConfigContains(oc, podname, "maxconn", []string{"maxconn 2000"})

		exutil.By("7: Patch tuningOptions/maxConnections with max 2000000 to IC ocp50926")
		patchResourceAsAdmin(oc, ingctrl.namespace, ingctrlResource, "{\"spec\": {\"tuningOptions\": {\"maxConnections\": 2000000}}}")
		ensureRouterDeployGenerationIs(oc, ingctrl.name, "3")

		exutil.By("8: Check ROUTER_MAX_CONNECTIONS env under a router pod of IC ocp50926,  which should be 2000000")
		podname = getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)
		maxConnSearch = readRouterPodEnv(oc, podname, "ROUTER_MAX_CONNECTIONS")
		o.Expect(maxConnSearch).To(o.ContainSubstring("ROUTER_MAX_CONNECTIONS=2000000"))

		exutil.By("9: Check maxconn in haproxy.config under a router pod of IC ocp50926, which should be 2000000")
		...
		exutil.By("10: Patch tuningOptions/maxConnections 0 to the IC")
		...
		exutil.By("11: Try to Check ROUTER_MAX_CONNECTIONS env in a route pod set to the default maxConnections by 0")
		podname = getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)
		cmd = fmt.Sprintf("/usr/bin/env | grep %s", "ROUTER_MAX_CONNECTIONS")
		_, err = oc.AsAdmin().WithoutNamespace().Run("exec").Args("-n", "openshift-ingress", podname, "--", "bash", "-c", cmd).Output()
		o.Expect(err).To(o.HaveOccurred())

		exutil.By("12: Check maxconn in haproxy.config under a router pod of the IC , which should be 50000")
		ensureHaproxyBlockConfigContains(oc, podname, "maxconn", []string{"maxconn 50000"})

		exutil.By("13: Try to patch the ingress-controller with tuningOptions/maxConnections 1999, which is less than the min 2000")
		...
		exutil.By("14: Try to patch the ingress-controller with tuningOptions/maxConnections 2000001, which is a larger than the max 2000000")
		NegMaxConnections = "2000001"
		output, err2 = oc.AsAdmin().WithoutNamespace().Run("patch").Args(ingctrlResource, "-p", "{\"spec\": {\"tuningOptions\": {\"maxConnections\": "+NegMaxConnections+"}}}", "--type=merge", "-n", ingctrl.namespace).Output()
		o.Expect(err2).To(o.HaveOccurred())
		o.Expect(output).To(o.ContainSubstring("Unsupported value: " + NegMaxConnections + ": supported values: \"-1\", \"0\""))

		exutil.By("15: Try to patch the ingress-controller with tuningOptions/maxConnections abc, which is a string")
	...
	g.It("Author:shudili-High-53605-Expose a Configurable Reload Interval in HAproxy", func() {
		...
		exutil.By("1: Create two custom ICs for testing router reload interval")
		baseDomain := getBaseDomain(oc)
		ingctrl1.domain = ingctrl1.name + "." + baseDomain
		ingctrl2.domain = ingctrl2.name + "." + baseDomain
		defer ingctrl1.delete(oc)
		...
		exutil.By("2: Check RELOAD_INTERVAL env in a route pod of IC ocp53605one, which should be 15s")
		podname1 := getOneNewRouterPodFromRollingUpdate(oc, ingctrl1.name)
		riSearch := readRouterPodEnv(oc, podname1, "RELOAD_INTERVAL")
		o.Expect(riSearch).To(o.ContainSubstring("RELOAD_INTERVAL=15s"))

		exutil.By("3: Check RELOAD_INTERVAL env in a route pod of IC ocp53605two, which should be 2m")
		podname2 := getOneNewRouterPodFromRollingUpdate(oc, ingctrl2.name)
		riSearch = readRouterPodEnv(oc, podname2, "RELOAD_INTERVAL")
		o.Expect(riSearch).To(o.ContainSubstring("RELOAD_INTERVAL=2m"))

		exutil.By("4: Patch tuningOptions/reloadInterval with other valid unit m, for exmpale 1m to IC ocp53605one, while patch it with 0s to IC ocp53605two")
		...
		exutil.By("5: Check RELOAD_INTERVAL env in a route pod of IC ocp53605one which should be 1m")
		podname1 = getOneNewRouterPodFromRollingUpdate(oc, ingctrl1.name)
		riSearch = readRouterPodEnv(oc, podname1, "RELOAD_INTERVAL")
		o.Expect(riSearch).To(o.ContainSubstring("RELOAD_INTERVAL=1m"))

		exutil.By("6: Check RELOAD_INTERVAL env in a route pod of IC ocp53605two, which is the default 5s")
		podname2 = getOneNewRouterPodFromRollingUpdate(oc, ingctrl2.name)
		riSearch = readRouterPodEnv(oc, podname2, "RELOAD_INTERVAL")
		...
		exutil.By("7: Try to patch tuningOptions/reloadInterval 121s which is larger than the max 120s, to the ingress-controller")
		...
		exutil.By("8: Check RELOAD_INTERVAL env in a route pod which should be the max 2m")
		podname := getOneNewRouterPodFromRollingUpdate(oc, ingctrl1.name)
		riSearch = readRouterPodEnv(oc, podname, "RELOAD_INTERVAL")
		o.Expect(riSearch).To(o.ContainSubstring("RELOAD_INTERVAL=2m"))

		exutil.By("9: Try to patch tuningOptions/reloadInterval 0.5s which is less than the min 1s, to the ingress-controller")
		NegReloadInterval = "0.5s"
		patchResourceAsAdmin(oc, ingctrl1.namespace, ingctrlResource1, "{\"spec\": {\"tuningOptions\": {\"reloadInterval\": \""+NegReloadInterval+"\"}}}")
		ensureRouterDeployGenerationIs(oc, ingctrl1.name, "4")

		exutil.By("10: Check RELOAD_INTERVAL env in a route pod which should be the min 1s")
		podname = getOneNewRouterPodFromRollingUpdate(oc, ingctrl1.name)
		riSearch = readRouterPodEnv(oc, podname, "RELOAD_INTERVAL")
		o.Expect(riSearch).To(o.ContainSubstring("RELOAD_INTERVAL=1s"))

		exutil.By("11: Try to patch tuningOptions/reloadInterval -1s which is a minus value, to the ingress-controller")
		...
		exutil.By("12: Try to patch tuningOptions/reloadInterval 1abc which is a string, to the ingress-controller")
		NegReloadInterval = "1abc"
		output, errCfg = patchResourceAsAdminAndGetLog(oc, ingctrl1.namespace, ingctrlResource1, "{\"spec\": {\"tuningOptions\": {\"reloadInterval\": \""+NegReloadInterval+"\"}}}")
		o.Expect(errCfg).To(o.HaveOccurred())
		o.Expect(output).To(o.ContainSubstring("Invalid value: \"" + NegReloadInterval + "\""))

		exutil.By("13: Try to patch tuningOptions/reloadInterval 012 s which contains a space character, to the ingress-controller")
	...
	g.It("[Level0] Author:shudili-High-55367-Default HAProxy maxconn value to 50000 for OCP 4.12 and later", func() {
		var (
			buildPruningBaseDir = testdata.FixturePath("router")
			customTemp          = filepath.Join(buildPruningBaseDir, "ingresscontroller-np.yaml")
			ingctrl             = ingressControllerDescription{
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-High-86153-Supporting HTTPKeepAliveTimeout tuning option", func() {
		...
		exutil.By("2.0: Check the default http-keep-alive in haproxy.config which should be 300s")
		podname := getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)
		ensureHaproxyBlockConfigContains(oc, podname, "defaults", []string{"http-keep-alive 300s"})

		exutil.By("2.1: Check default value of ROUTER_SLOWLORIS_HTTP_KEEPALIVE env in a route pod, which shouldn't appear")
		cmd := fmt.Sprintf("/usr/bin/env | grep %s", "ROUTER_SLOWLORIS_HTTP_KEEPALIVE")
		_, err := oc.AsAdmin().WithoutNamespace().Run("exec").Args("-n", "openshift-ingress", podname, "--", "bash", "-c", cmd).Output()
		o.Expect(err).To(o.HaveOccurred())

		exutil.By("3.0: Patch tuningOptions/httpKeepAliveTimeout with 50s to the ingress-controller")
		patchResourceAsAdmin(oc, ingctrl.namespace, ingctrlResource, `{"spec": {"tuningOptions": {"httpKeepAliveTimeout": "50s"}}}`)
		ensureRouterDeployGenerationIs(oc, ingctrl.name, "2")

		exutil.By("3.1: Check the tuningOptions.httpKeepAliveTimeout in the ingress-controller which should be 50s")
		...
		exutil.By("3.2: Check http-keep-alive in haproxy.config which should be 50s")
		podname = getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)
		ensureHaproxyBlockConfigContains(oc, podname, "defaults", []string{"http-keep-alive 50s"})

		exutil.By("3.3: Check ROUTER_SLOWLORIS_HTTP_KEEPALIVE env in a route pod which should be 50s")
		podname = getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)
		...
		exutil.By("4.0: Try to patch tuningOptions/httpKeepAliveTimeout with 50m, which is larger than the max 15m to the ingress-controller")
		...
		exutil.By("5.0: Try to patch tuningOptions/httpKeepAliveTimeout with 1h, the unit of which is not supported")
