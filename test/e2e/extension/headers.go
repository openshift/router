...
var _ = g.Describe("[OTP][sig-network-edge] Network_Edge Component_Router", func() {
	...
	var oc = exutil.NewCLI("router-headers", exutil.KubeConfigPath())
...
`)
		...
		exutil.By("2.0 Create a client pod, a backend pod and its service resources")
		ns := oc.Namespace()
		...
		curlCmd := []string{"-n", ns, clientPodName, "--", "curl", "http://" + routehost + "/headers", "-I", "--resolve", toDst, "--connect-timeout", "10"}
...
`)
		...
		exutil.By("1.0 Create an custom IC with forwardedHeaderPolicy Append")
		baseDomain := getBaseDomain(oc)
		ingctrl.domain = ingctrl.name + "." + baseDomain
		defer ingctrl.delete(oc)
		ingctrl.create(oc)
		...
		exutil.By("3.0: Create a http route for testing OCP-34231")
		...
		exutil.By("4.0: Check the forwardedHeaderPolicy env, which should be append")
		...
		exutil.By("5.0: Curl the http route with a specified X-Forwarded-Host, then check the X-Forwarded-Host header value, which should contain both the exampleXForwardedHost and routehost")
		podIP := getPodv4Address(oc, routerpod, "openshift-ingress")
		toDst := routehost + ":80:" + podIP
		...
		exutil.By("6.0: Create two http routes for testing OCP-34247")
		...
		exutil.By("7.0 Add the set-forwarded-headers annotations to the two routes, one with replace, the other with never, then check it in haproxy")
		...
		exutil.By("8.0: Curl the replace annotation http route with a specified X-Forwarded-Host, then check the header value which should be replaced by routehostOCP34247a")
		...
		exutil.By("9.0: Curl the never annotation http route, then check the http headers which should not contain the X-Forwarded-Host header")
...
`)
		...
		exutil.By("1.0 Create an custom IC with forwardedHeaderPolicy Replace")
		baseDomain := getBaseDomain(oc)
		ingctrl.domain = ingctrl.name + "." + baseDomain
		defer ingctrl.delete(oc)
		ingctrl.create(oc)
		...
		exutil.By("3.0: Create a http route for testing OCP-34233")
		routehost := "unsecure34233" + "." + ingctrl.domain
		createRoute(oc, ns, "http", "route-http", unsecSvcName, []string{"--hostname=" + routehost})
		ensureRouteIsAdmittedByIngressController(oc, ns, "route-http", ingctrl.name)

		exutil.By("4.0: Check the forwardedHeaderPolicy env, which should be replace")
		routerpod := getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)
		pollReadPodData(oc, "openshift-ingress", routerpod, "/usr/bin/env", `ROUTER_SET_FORWARDED_HEADERS=replace`)

		exutil.By("5.0: Curl the http route, then check the X-Forwarded-Host header value: exampleXForwardedHost should be replaced by routehost")
		podIP := getPodv4Address(oc, routerpod, "openshift-ingress")
		toDst := routehost + ":80:" + podIP
		curlCmd := []string{"-n", ns, clientPodName, "--", "curl", "http://" + routehost + "/headers", "-sv", "-H", "X-Forwarded-Host: " + exampleXForwardedHost, "--resolve", toDst, "--connect-timeout", "10"}
		output, _ := repeatCmdOnClient(oc, curlCmd, "200 OK", 60, 1)
		...
		exutil.By("6.0: Create a http route for the testing OCP-34246")
		...
		exutil.By("7.0 Add set-forwarded-headers=if-none annotation to the route, then check it in haproxy")
		...
		exutil.By("8.0: Curl the http route with a specified X-Forwarded-Host, then check the X-Forwarded-Host header value, which should not be replaced by routehost or not be appended by routehost")
		toDst = routehost + ":80:" + podIP
		curlCmd = []string{"-n", ns, clientPodName, "--", "curl", "http://" + routehost + "/headers", "-sv", "-H", "X-Forwarded-Host: " + exampleXForwardedHost, "--resolve", toDst, "--connect-timeout", "10"}
		output, _ = repeatCmdOnClient(oc, curlCmd, "200 OK", 60, 1)
		...
		exutil.By("9.0: Curl the http route with a specified X-Forwarded-Proto-Version, then check the header value is added")
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-High-34234-Configure Ingresscontroller to set the headers if they are not already set with forwardedHeaderPolicy set to Ifnone", func() {
		...
		exutil.By("1.0 Create an custom IC with default forwardedHeaderPolicy Append")
		baseDomain := getBaseDomain(oc)
		ingctrl.domain = ingctrl.name + "." + baseDomain
		defer ingctrl.delete(oc)
		ingctrl.create(oc)
		...
		exutil.By("5.0: Curl the http route with specified X-Forwarded-Host, then check the X-Forwarded-Host header value, which should contain both the exampleXForwardedHost and routehost")
		podIP := getPodv4Address(oc, routerpod, "openshift-ingress")
		toDst := routehost + ":80:" + podIP
		curlCmd := []string{"-n", ns, clientPodName, "--", "curl", "http://" + routehost + "/headers", "-sv", "-H", "X-Forwarded-Host: " + exampleXForwardedHost, "--resolve", toDst, "--connect-timeout", "10"}
		output, _ := repeatCmdOnClient(oc, curlCmd, "200 OK", 60, 1)
		...
		exutil.By("6.0 Patch the httpHeaders with forwardedHeaderPolicy IfNone")
		...
		exutil.By("7.0: Check httpCaptureHeaders configuration in haproxy")
		routerpod = getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)
		...
		exutil.By("10.0 Patch the httpHeaders with forwardedHeaderPolicy Never")
		patchPath = `{"spec":{"httpHeaders":{"forwardedHeaderPolicy":"Never"}}}`
		patchResourceAsAdmin(oc, ingctrl.namespace, ingctrlResource, patchPath)
		ensureRouterDeployGenerationIs(oc, ingctrl.name, "3")

		exutil.By("11.0: Check httpCaptureHeaders configuration in haproxy")
		routerpod = getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)
		pollReadPodData(oc, "openshift-ingress", routerpod, "/usr/bin/env", `ROUTER_SET_FORWARDED_HEADERS=never`)

		exutil.By("12.0: Curl the http route with a specified X-Forwarded-Host, then check the http headers which should not contain it")
		podIP = getPodv4Address(oc, routerpod, "openshift-ingress")
		toDst = routehost + ":80:" + podIP
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-ConnectedOnly-High-62528-adding/deleting http headers to an edge route by a router owner", func() {
		...
		exutil.By("Create a client pod, a backend pod and its service resources")
		ns := oc.Namespace()
		err = oc.AsAdmin().WithoutNamespace().Run("create").Args("-n", ns, "-f", clientPod).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
		ensurePodWithLabelReady(oc, ns, clientPodLabel)
		err = oc.AsAdmin().WithoutNamespace().Run("cp").Args("-n", ns, fileDir, ns+"/"+clientPodName+":"+podFileDir).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
		createResourceFromFile(oc, ns, testPodSvc)
		ensurePodWithLabelReady(oc, ns, "name=httpbin-pod")
		...
		err = oc.AsAdmin().WithoutNamespace().Run("create").Args("-n", ns, "route", "edge", "r3-edge", "--service="+unsecsvcName, "--cert="+customCert, "--key="+customKey, "--ca-cert="+name+"-ca.pem", "--hostname="+edgeRouteHost).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		output, err := oc.AsAdmin().WithoutNamespace().Run("patch").Args("route/r3-edge", "-p", patchHeaders, "--type=merge", "-n", ns).Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(output).To(o.ContainSubstring("patched"))

		exutil.By("check backend edge route in haproxy that headers to be set or deleted")
		routeBackendCfg := ensureHaproxyBlockConfigContains(oc, routerpod, "be_edge_http:"+ns+":r3-edge", []string{"X-SSL-Client-Cert"})
		...
		curlEdgeRouteReq := []string{"-n", ns, clientPodName, "--", "curl", "https://" + edgeRouteHost + "/headers", "-v", "--cacert", podCaCert, "--cert", podCustomCert, "--key", podCustomKey, "--resolve", edgeRouteDst, "--connect-timeout", "10"}
		curlEdgeRouteRes := []string{"-n", ns, clientPodName, "--", "curl", "https://" + edgeRouteHost + "/headers", "-I", "--cacert", podCaCert, "--cert", podCustomCert, "--key", podCustomKey, "--resolve", edgeRouteDst, "--connect-timeout", "10"}
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-ConnectedOnly-High-66560-adding/deleting http headers to a http route by a router owner", func() {
		...
		exutil.By("Expose a route with the unsecure service inside the namespace")
		...
		output, err := oc.AsAdmin().WithoutNamespace().Run("patch").Args("route/"+unsecsvcName, "-p", patchHeaders, "--type=merge", "-n", ns).Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(output).To(o.ContainSubstring("patched"))

		exutil.By("check backend edge route in haproxy that headers to be set or deleted")
		routerpod := getOneRouterPodNameByIC(oc, ingctrl.name)
		routeBackendCfg := ensureHaproxyBlockConfigContains(oc, routerpod, "be_http:"+ns+":"+unsecsvcName, []string{"X-SSL-Client-Cert"})
		o.Expect(strings.Contains(routeBackendCfg, "http-request set-header 'X-SSL-Client-Cert' '%{+Q}[ssl_c_der,base64]'")).To(o.BeTrue())
		o.Expect(strings.Contains(routeBackendCfg, "http-request set-header 'X-Target' '%[req.hdr(host),lower]'")).To(o.BeTrue())
		o.Expect(strings.Contains(routeBackendCfg, "http-request set-header 'reqTestHost1' '%[req.hdr(host),lower]'")).To(o.BeTrue())
		o.Expect(strings.Contains(routeBackendCfg, "http-request set-header 'reqTestHost2' '%[req.hdr(host),base64]'")).To(o.BeTrue())
		...
		podIP := getPodv4Address(oc, routerpod, "openshift-ingress")
		toDst := routeHost + ":80:" + podIP
		curlHTTPRouteReq := []string{"-n", ns, clientPodName, "--", "curl", "http://" + routeHost + "/headers", "-v", "-e", "www.qe-test.com", "--resolve", toDst, "--connect-timeout", "10"}
		curlHTTPRouteRes := []string{"-n", ns, clientPodName, "--", "curl", "http://" + routeHost + "/headers", "-I", "-e", "www.qe-test.com", "--resolve", toDst, "--connect-timeout", "10"}
		lowSrv := strings.ToLower(srv)
		base64Srv := base64.StdEncoding.EncodeToString([]byte(srv))
	...
	g.It("Author:shudili-DEPRECATED-ROSA-OSD_CCS-ARO-ConnectedOnly-Medium-66566-supported max http headers, max length of a http header name, max length value of a http header", func() {
		...
		patchResourceAsAdmin(oc, ns, "route/"+unsecsvcName, patchHeaders)
		routeBackend := "be_http:" + ns + ":" + unsecsvcName
		routerpod := getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)
		podIP := getPodv4Address(oc, routerpod, "openshift-ingress")
		toDst := routehost + ":80:" + podIP
		...
		cmdOnPod := []string{"-n", ns, clientPodName, "--", "curl", "-Is", "http://" + routehost + "/headers", "--resolve", toDst, "--connect-timeout", "10"}
		repeatCmdOnClient(oc, cmdOnPod, "200", 60, 1)
		resHeaders, err := oc.Run("exec").Args("-n", ns, clientPodName, "--", "curl", "-s", "http://"+routehost+"/headers", "--resolve", toDst, "--connect-timeout", "10").Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		patchResourceAsAdmin(oc, ns, "route/"+unsecsvcName, "{\"spec\": {\"httpHeaders\": null}}")
		output, err := oc.AsAdmin().WithoutNamespace().Run("patch").Args("route/"+unsecsvcName, "-p", negPatchHeaders, "--type=merge", "-n", ns).Output()
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-ConnectedOnly-Medium-66568-negative test of adding/deleting http headers", func() {
		...
		o.Expect(err).To(o.HaveOccurred())
		o.Expect(output).To(o.MatchRegexp("Duplicate value:.+testheader1"))
		...
		output, err = oc.AsAdmin().WithoutNamespace().Run("patch").Args("route/"+unsecsvcName, "-p", proxyHeader, "--type=merge", "-n", ns).Output()
		o.Expect(err).To(o.HaveOccurred())
		...
		err = oc.AsAdmin().WithoutNamespace().Run("patch").Args("route/"+unsecsvcName, "-p", hostHeader, "--type=merge", "-n", ns).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
		jpath := `{.spec.httpHeaders.actions.request[?(@.name=="host")].action.set.value}`
		host := getByJsonPath(oc, ns, "route/"+unsecsvcName, jpath)
		...
		output, err = oc.AsAdmin().WithoutNamespace().Run("patch").Args("route/"+unsecsvcName, "-p", hstsHeader, "--type=merge", "-n", ns).Output()
		o.Expect(err).To(o.HaveOccurred())
		o.Expect(output).To(o.ContainSubstring("Forbidden: the following headers may not be modified using this API: strict-transport-security, proxy, cookie, set-cookie"))
		...
		output, err = oc.AsAdmin().WithoutNamespace().Run("patch").Args("route/"+unsecsvcName, "-p", cookieHeader, "--type=merge", "-n", ns).Output()
		o.Expect(err).To(o.HaveOccurred())
		o.Expect(output).To(o.ContainSubstring("Forbidden: the following headers may not be modified using this API: strict-transport-security, proxy, cookie, set-cookie"))
		...
		output, err = oc.AsAdmin().WithoutNamespace().Run("patch").Args("route/"+unsecsvcName, "-p", setCookieHeader, "--type=merge", "-n", ns).Output()
		o.Expect(err).To(o.HaveOccurred())
		o.Expect(output).To(o.ContainSubstring("Forbidden: the following headers may not be modified using this API: strict-transport-security, proxy, cookie, set-cookie"))
		...
		err = oc.AsAdmin().WithoutNamespace().Run("patch").Args("route/"+unsecsvcName, "-p", routeHeaders, "--type=merge", "-n", ns).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		podIP := getPodv4Address(oc, routerpod, "openshift-ingress")
		toDst := routehost + ":80:" + podIP
		cmdOnPod := []string{"-n", ns, clientPodName, "--", "curl", "-I", "http://" + routehost + "/headers", "--resolve", toDst, "--connect-timeout", "10"}
		repeatCmdOnClient(oc, cmdOnPod, "200", 60, 1)
		reqHeaders, err := oc.Run("exec").Args("-n", ns, clientPodName, "--", "curl", "http://"+routehost+"/headers", "--resolve", toDst, "--connect-timeout", "10").Output()
		o.Expect(err).NotTo(o.HaveOccurred())
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-NonPreRelease-Longduration-ConnectedOnly-Medium-66569-set different type of values for a http header name and its value", func() {
		...
		exutil.By("Create a backend pod and its service resources")
		ns := oc.Namespace()
		createResourceFromFile(oc, ns, testPodSvc)
		ensurePodWithLabelReady(oc, ns, "name=httpbin-pod")
		...
		patchResourceAsAdmin(oc, ns, routeResource, validHeaders)
		routerpod := getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)
		routeBackend := "be_http:" + ns + ":" + unsecsvcName
		...
		output, err := oc.AsAdmin().WithoutNamespace().Run("patch").Args("route/"+unsecsvcName, "-p", blankHeaderName, "--type=merge", "-n", ns).Output()
		o.Expect(err).To(o.HaveOccurred())
		...
		patchResourceAsAdmin(oc, ns, routeResource, specialHeaderName1)
		...
		patchResourceAsAdmin(oc, ns, routeResource, specialHeaderName2)
		ensureHaproxyBlockConfigContains(oc, routerpod, routeBackend, []string{"http-request set-header 'aabbccdd'\\''ee' 'abc'"})
		...
		output, err = oc.AsAdmin().WithoutNamespace().Run("patch").Args("route/"+unsecsvcName, "-p", specialHeaderName3, "--type=merge", "-n", ns).Output()
		o.Expect(err).To(o.HaveOccurred())
		...
		patchResourceAsAdmin(oc, ns, routeResource, specialHeaderValues)
	...
	g.It("Author:shudili-DEPRECATED-ROSA-OSD_CCS-ARO-ConnectedOnly-High-66572-adding/deleting http headers to a http route by an ingress-controller as a cluster administrator", func() {
		...
		curlHTTPRouteReq := []string{"-n", ns, clientPodName, "--", "curl", "http://" + routeHost + "/headers", "-v", "-e", "www.qe-test.com", "--resolve", routeDst, "--connect-timeout", "10"}
		curlHTTPRouteRes := []string{"-n", ns, clientPodName, "--", "curl", "http://" + routeHost + "/headers", "-I", "-e", "www.qe-test.com", "--resolve", routeDst, "--connect-timeout", "10"}
		lowSrv := strings.ToLower(srv)
		base64Srv := base64.StdEncoding.EncodeToString([]byte(srv))
		repeatCmdOnClient(oc, curlHTTPRouteRes, "200", 60, 1)
...
`, secsvcName, ns)
		...
		err = oc.AsAdmin().WithoutNamespace().Run("cp").Args("-n", ns, fileDir, ns+"/"+clientPodName+":"+podFileDir).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
		operateResourceFromFile(oc, "create", ns, serverPod)
		ensurePodWithLabelReady(oc, ns, "name=httpbin-pod")
		createResourceFromFile(oc, ns, secsvc)

		exutil.By("Update the certification and key in the server pod")
		podName := getPodListByLabel(oc, ns, "name=httpbin-pod")
		newSrvCert := ns + "/" + podName[0] + ":" + srvCert
		newSrvKey := ns + "/" + podName[0] + ":" + srvKey
		_, err = oc.AsAdmin().WithoutNamespace().Run("exec").Args("-n", ns, podName[0], "-c", "httpbin-https", "--", "bash", "-c", "rm -f "+srvCert).Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		_, err = oc.AsAdmin().WithoutNamespace().Run("exec").Args("-n", ns, podName[0], "-c", "httpbin-https", "--", "bash", "-c", "rm -f "+srvKey).Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		err = oc.AsAdmin().WithoutNamespace().Run("cp").Args("-n", ns, destCA, "-c", "httpbin-https", newSrvCert).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
		err = oc.AsAdmin().WithoutNamespace().Run("cp").Args("-n", ns, destKey, "-c", "httpbin-https", newSrvKey).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		err = oc.AsAdmin().WithoutNamespace().Run("create").Args("-n", ns, "route", "reencrypt", "r2-reen", "--service="+secsvcName, "--cert="+customCert, "--key="+customKey, "--ca-cert="+name+"-ca.pem", "--dest-ca-cert="+destCA, "--hostname="+reenRouteHost).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		output, err := oc.AsAdmin().WithoutNamespace().Run("patch").Args("route/r2-reen", "-p", patchHeaders, "--type=merge", "-n", ns).Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(output).To(o.ContainSubstring("patched"))

		exutil.By("check backend reen route in haproxy that headers to be set or deleted")
		routeBackendCfg := ensureHaproxyBlockConfigContains(oc, routerpod, "be_secure:"+ns+":r2-reen", []string{"X-SSL-Client-Cert"})
		o.Expect(strings.Contains(routeBackendCfg, "http-request set-header 'X-SSL-Client-Cert' '%{+Q}[ssl_c_der,base64]'")).To(o.BeTrue())
		o.Expect(strings.Contains(routeBackendCfg, "http-request set-header 'X-Target' '%[req.hdr(host),lower]'")).To(o.BeTrue())
		o.Expect(strings.Contains(routeBackendCfg, "http-request set-header 'reqTestHost1' '%[req.hdr(host),lower]'")).To(o.BeTrue())
		o.Expect(strings.Contains(routeBackendCfg, "http-request set-header 'reqTestHost2' '%[req.hdr(host),base64]'")).To(o.BeTrue())
		...
		curlReenRouteReq := []string{"-n", ns, clientPodName, "--", "curl", "https://" + reenRouteHost + "/headers", "-v", "--cacert", podCaCert, "--cert", podCustomCert, "--key", podCustomKey, "--resolve", reenRouteDst, "--connect-timeout", "10"}
		curlReenRouteRes := []string{"-n", ns, clientPodName, "--", "curl", "https://" + reenRouteHost + "/headers", "-I", "--cacert", podCaCert, "--cert", podCustomCert, "--key", podCustomKey, "--resolve", reenRouteDst, "--connect-timeout", "10"}
		lowSrv := strings.ToLower(srv)
		base64Srv := base64.StdEncoding.EncodeToString([]byte(srv))
...
`, secsvcName, ns)
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-ConnectedOnly-High-77284-http request with duplicated headers should not cause disruption to a router pod", func() {
		...
		exutil.By("3.0 Create a HTTP route inside the namespace")
		...
		podIP := getPodv4Address(oc, routerpod, "openshift-ingress")
		toDst := routehost + ":80:" + podIP
		cmdOnPod := []string{"-n", ns, clientPodName, "--", "curl", "-I", "http://" + routehost + "/headers", "-H", `"transfer-encoding: chunked"`, "-H", `"transfer-encoding: chunked"`, "--resolve", toDst, "--connect-timeout", "10"}
