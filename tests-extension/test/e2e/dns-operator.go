...
var _ = g.Describe("[sig-network-edge][OTP] Network_Edge Component_DNS", func() {
	defer g.GinkgoRecover()
	var oc = compat_otp.NewCLI("dns-operator", compat_otp.KubeConfigPath())
	...
	g.It("Author:mjoseph-NonHyperShiftHOST-High-37912-DNS operator should show clear error message when DNS service IP already allocated [Disruptive]", func() {
		...
		compat_otp.By("Step1: Scale the CVO and DNS operator pod to zero and delete the default DNS service")
		...
		compat_otp.By("Step2: Create a test server with the Cluster IP and scale up the dns operator")
		...
		compat_otp.By("Step3: Confirm the new dns service came with the given address")
		...
		compat_otp.By("Step4: Confirm the error message from the DNS operator status")
		...
		compat_otp.By("Step5: Check the degraded status of dns operator among the cluster operators")
		...
		waitForOutputEquals(oc, "default", "co/dns", jsonPath, "FalseTrueTrue")
	})
	...
	g.It("Author:mjoseph-Critical-41049-DNS controlls pod placement by node selector [Disruptive]", func() {
		...
		compat_otp.By("check the default dns nodeSelector is present")
		...
		compat_otp.By("check the nodeSelector can be updated")
		...
		compat_otp.By("check the dns pod is running on the expected node")
	...
	g.It("Author:mjoseph-Critical-41050-DNS controll pod placement by tolerations [Disruptive]", func() {
		...
		compat_otp.By("check dns pod placement to confirm it is running on default tolerations")
		...
		compat_otp.By("Patch dns operator config with custom tolerations of dns pod, not to tolerate master node taints")
		...
		compat_otp.By("check dns pod placement to check the custom tolerations")
		tolerationCfg = getByJsonPath(oc, ns, "ds/dns-default", "{.spec.template.spec.tolerations}")
		o.Expect(tolerationCfg).To(o.ContainSubstring(`{"effect":"NoExecute","key":"my-dns-test","operator":"Equal","value":"abc"}`))

		compat_otp.By("check dns.operator status to see any error messages")
	...
	g.It("Author:hongli-High-46183-DNS operator supports Random, RoundRobin and Sequential policy for servers.forwardPlugin [Disruptive]", func() {
		...
		compat_otp.By("patch the dns.operator/default and add custom zones config, check Corefile and ensure the policy is Random")
		...
		compat_otp.By("updateh the custom zones policy to RoundRobin, check Corefile and ensure it is updated ")
		...
		compat_otp.By("updateh the custom zones policy to Sequential, check Corefile and ensure it is updated")
	...
	g.It("Author:shudili-NonHyperShiftHOST-Medium-46873-Configure operatorLogLevel under the default dns operator and check the logs flag [Disruptive]", func() {
		...
		compat_otp.By("Check default log level of dns operator")
		...
		compat_otp.By("Remove dns operator pod")
		dnsOperatorPodName := getPodListByLabel(oc, "openshift-dns-operator", "name=dns-operator")[0]
		...
		compat_otp.By("Patch dns operator with operator logLevel Debug")
		...
		compat_otp.By("Patch dns operator with operator logLevel trace")
		...
		compat_otp.By("Patch dns operator with operator logLevel normal")
		...
		compat_otp.By("Check logs of dns operator")
