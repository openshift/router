...
var _ = g.Describe("[sig-network-edge][OTP][Level0] Network_Edge Component_Router", func() {
	...
	g.It("Author:mjoseph-ROSA-OSD_CCS-ARO-Critical-22636-The namespaceSelector of router is controlled by ingresscontroller", func() {
		...
		compat_otp.By("5. Check the haproxy config on the custom router pod to find the backend details of the " + ns + " route")
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-Medium-30264-ROUTER_SYSLOG_ADDRESS changes according to the logging configuration", func() {
		...
		compat_otp.By("5. check custom ingress pod for ROUTER_SYSLOG_ADDRESS")
		...
		compat_otp.By("6. Patch the ingress controller with a different address and port")
		...
		compat_otp.By("7. Wait for previous pod to be deleted")
		err = waitForResourceToDisappear(oc, "openshift-ingress", "pod/"+ingPod)
		o.Expect(err).NotTo(o.HaveOccurred())

		compat_otp.By("8. Retrieve new pod name with label")
		ingPod = getOnePodNameByLabel(oc, "openshift-ingress", "ingresscontroller.operator.openshift.io/deployment-ingresscontroller="+ingctrl.name)
		o.Expect(ingPod).To(o.ContainSubstring(ingctrl.name))

		compat_otp.By("9. check custom ingress pod for ROUTER_SYSLOG_ADDRESS")
		output, err = oc.AsAdmin().WithoutNamespace().Run("exec").Args("-n", "openshift-ingress", ingPod, "-", "bash", "-lc", cmd).Output()
		o.Expect(err).NotTo(o.HaveOccurred())
...
`)
	...
	// https://issues.redhat.com/browse/OCPBUGS-60885
	// author: shudili@redhat.com
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-High-86155-Supporting ClosedClientConnectionPolicy in the IngressController", func() {
		...
		compat_otp.By("2.0: Check the default closedClientConnectionPolicy in the ingress-controller which should be Continue")
		...
		compat_otp.By("2.1: Check in the haproxy.conf there is NOT option abortonclose")
		...
		compat_otp.By("2.2: Check in the haproxy-config.template whether or not there is option abortonclose")
		...
		compat_otp.By("3.0: Patch the ingress-controller with Abort closedClientConnectionPolicy")
		patchResourceAsAdmin(oc, ingctrl.namespace, ingctrlResource, `{"spec":{"closedClientConnectionPolicy":"Abort"}}`)
		ensureRouterDeployGenerationIs(oc, ingctrl.name, "2")

		compat_otp.By("3.1: Check the spec of the ingress-controller, closedClientConnectionPolicy should be Abort")
		policy = getByJsonPath(oc, ingctrl.namespace, ingctrlResource, "{.spec.closedClientConnectionPolicy}")
		o.Expect(policy).To(o.ContainSubstring("Abort"))

		compat_otp.By("3.2: Check in the haproxy.config whether or not there is option abortonclose")
		podname = getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)
		ensureHaproxyBlockConfigContains(oc, podname, "defaults", []string{"option abortonclose"})

		compat_otp.By("3.3: Check ROUTER_ABORT_ON_CLOSE env in a route pod which should be true")
