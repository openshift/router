...
var _ = g.Describe("[sig-network-edge][OTP] Network_Edge", func() {
...
ingress:
    accessLogging:
		...
		compat_otp.By("5.0: Create a http/edge/reen route for the testing")
		...
		compat_otp.By("8.1: Curl the http route with cookie foo22=bar22, expect to get 200 OK")
		...
		compat_otp.By("8.2: Curl the edge route with cookie foo=barforedge, expect to get 200 OK")
		...
		compat_otp.By("8.3: Curl the reen route with cookie foo=barforreen, expect to get 200 OK")
		...
		compat_otp.By("11.0: Check the router logs, which should contain both the cookie foo22=bar22 and the url")
		logs = waitRouterLogsAppear(oc, routerpod, "foo22=bar22")
		o.Expect(logs).To(o.ContainSubstring("index.html"))

		// check logs for edge route of OCP-84261
		compat_otp.By("12.0: Check the router logs, which should contain both the cookie foo=barforedge and the url for the edge route")
		logs = waitRouterLogsAppear(oc, routerpod, "foo=barforedge")
		o.Expect(logs).To(o.ContainSubstring("index.html"))

		// check logs for reen route of OCP-84261
		compat_otp.By("13.0: Check the router logs, which should contain both the cookie foo=barforreen and the url for the reen route")
...
ingress:
    accessLogging:
		...
		compat_otp.By("6.0: Check httpCaptureHeaders configuration in haproxy")
		routerpod := getOneNewRouterPodFromRollingUpdate(oc, "default")
		...
		compat_otp.By("8.0: Check the router logs, which should contain both the request host and the response server")
		...
		compat_otp.By("9.0: Check the router logs, which should contain both the request host of the edge route and the response server")
		logs = waitRouterLogsAppear(oc, routerpod, edgeRouteHost)
		o.Expect(logs).To(o.MatchRegexp(server + "/" + "[0-9\\.]+"))

		// check logs for reen route of OCP-84259
		compat_otp.By("10.0: Check the router logs, which should contain both the request host of the reen route and the response server")
	...
	g.It("Author:shudili-MicroShiftOnly-High-82003-The httpCaptureHeaders option strictly adheres to the maxlength parameter [Disruptive]", func() {
		...
		compat_otp.By("3.0: Debug node and configure a small maxLength for request/response httpCaptureHeaders")
		ingressConfig := fmt.Sprintf(`
ingress:
    accessLogging:
        httpCaptureHeaders:
		...
		compat_otp.By("6.0: Curl the http route, expect to get 200 OK")
		routerpod := getOneNewRouterPodFromRollingUpdate(oc, "default")
		podIP := getPodv4Address(oc, routerpod, "openshift-ingress")
		toDst := routehost + ":80:" + podIP
		curlCmd := []string{"-n", e2eTestNamespace, clientPodName, "--", "curl", "http://" + routehost + "/index.html", "-sI", "--resolve", toDst, "--connect-timeout", "10"}
		...
		compat_otp.By("7.0: Check the router logs, which should contain the request host header and response server header with values not exceeding the maxLength")
	...
	g.It("Author:shudili-MicroShiftOnly-High-82004-custom http 503 and 404 error pages for ingress route [Disruptive]", func() {
		...
		compat_otp.By("1.0  Create a configmap with the customized error pages in openshift-ingress namespace")
		...
		compat_otp.By("2.0: Prepare a namespace for the following testing")
		defer oc.DeleteSpecifiedNamespaceAsAdmin(e2eTestNamespace)
		oc.CreateSpecifiedNamespaceAsAdmin(e2eTestNamespace)
		compat_otp.SetNamespacePrivileged(oc, e2eTestNamespace)

		compat_otp.By("3.0: Debug node to backup the config.yaml, and restore it before the test finishes running")
		nodeName := getByJsonPath(oc, "default", "nodes", "{.items[0].metadata.name}")
		defer restoreConfigYaml(oc, e2eTestNamespace, caseID, nodeName)
		backupConfigYaml(oc, e2eTestNamespace, caseID, nodeName)
		actualGenerationInt := getRouterDeploymentGeneration(oc, "router-default")

		compat_otp.By("4.0: Debug node and configure custom ingress httpErrorCodePages")
		ingressConfig := fmt.Sprintf(`
ingress:
    httpErrorCodePages:
        name: %s
`, configmapName)
		...
		compat_otp.By("5.0: Create a client pod, a deployment and the services")
		createResourceFromFile(oc, e2eTestNamespace, clientPod)
		ensurePodWithLabelReady(oc, e2eTestNamespace, clientPodLabel)
		createResourceFromFile(oc, e2eTestNamespace, testPodSvc)
		ensurePodWithLabelReady(oc, e2eTestNamespace, "name="+srvrcInfo)

		compat_otp.By("6.0: Create a http route for the testing")
		createRoute(oc, e2eTestNamespace, "http", "route-http", unsecSvcName, []string{"--hostname=" + routehost})
		ensureRouteIsAdmittedByIngressController(oc, e2eTestNamespace, "route-http", "default")

		compat_otp.By("7.0: Check customized error 503 and 404 pages in haproxy")
		routerpod := getOneNewRouterPodFromRollingUpdate(oc, "default")
		...
		compat_otp.By("8.0: Curl a not exist http route, expect to get the custom 404 error page")
		podIP := getPodv4Address(oc, routerpod, "openshift-ingress")
		...
		compat_otp.By("9.0: Scale down the deployment to 0, the curl a http route, expect to get the custom 503 error page")
	...
	g.It("Author:shudili-MicroShiftOnly-NonPreRelease-High-82014-httpLogFormat for logging http requests [Disruptive]", func() {
		...
		compat_otp.By(`3.0: Debug node and configure httpLogFormat with %{+Q}r`)
		val := `%{+Q}r`
		ingressConfig := fmt.Sprintf(`
ingress:
    accessLogging: 
...
`, val)
		...
		compat_otp.By("6.0: Check httpLogFormat configuration in haproxy")
		routerpod := getOneNewRouterPodFromRollingUpdate(oc, "default")
		...
		compat_otp.By("8.0: Check the router logs, which should contain the desired info including the http request url")
		...
		compat_otp.By(`9.0: Debug node and configure httpLogFormat with %ci:%cp %si:%sp %HU %ST`)
...
`, val)
		...
		compat_otp.By("10.0: Check new httpLogFormat configuration in haproxy")
		routerpod = getOneNewRouterPodFromRollingUpdate(oc, "default")
		// OCPBUGS-60257(indentation issue of "frontend public" in haproxy)
		// formatCfg = getBlockConfig(oc, routerpod, "defaults")
		// expectedVal = fmt.Sprintf(`log-format "%s"`, val)
		...
		compat_otp.By("11.0: Curl the http route, expect to get 200 OK")
		podIP = getPodv4Address(oc, routerpod, "openshift-ingress")
		toDst = routehost + ":80:" + podIP
		curlCmd = []string{"-n", e2eTestNamespace, clientPodName, "--", "curl", "http://" + routehost + "/path/second/index.html", "-sI", "--resolve", toDst, "--connect-timeout", "10"}
		repeatCmdOnClient(oc, curlCmd, "200 OK", 90, 6)

		compat_otp.By("12.0: Check the router logs, which should contain the desired info including source & destination address and http status code")
		logs = waitRouterLogsAppear(oc, routerpod, "/path/second/index.html")
	...
	g.It("Author:shudili-MicroShiftOnly-NonPreRelease-High-82015-logging destination to a syslog server [Disruptive]", func() {
		...
		compat_otp.By("2.0: Create a client pod, a rsyslogd pod, a deployment and the services")
		createResourceFromFile(oc, e2eTestNamespace, clientPod)
		ensurePodWithLabelReady(oc, e2eTestNamespace, clientPodLabel)
		...
		compat_otp.By(`4.0: Debug node and configure the syslog destination`)
...
`, syslogIP)
		...
		compat_otp.By("6.0: Check the syslog configuration in haproxy")
		routerpod := getOneNewRouterPodFromRollingUpdate(oc, "default")
		...
		compat_otp.By("8.0: Check the logs of syslog pod, which should contain the haproxy log")
		...
		compat_otp.By(`9.0: Debug node and configure destination syslog with facility local2`)
		ingressConfig = fmt.Sprintf(`
ingress:
    accessLogging:
        destination:
...
`, syslogIP)
		...
		compat_otp.By("10.0: Check the new syslog configuration in haproxy")
		routerpod = getOneNewRouterPodFromRollingUpdate(oc, "default")
		syslogCfg = getBlockConfig(oc, routerpod, "global")
	...
	g.It("Author:shudili-MicroShiftOnly-Medium-84260-Negative test of the logging configuration [Disruptive]", func() {
		...
		compat_otp.By(`3.0: Debug node and try to configure invalid maxLength -1 of the httpCaptureCookies`)
		ingressConfig := fmt.Sprintf(`ingress:
    accessLogging:
        httpCaptureCookies:
            - matchType: Exact
		...
		compat_otp.By(`4.0: Debug node and try to configure invalid maxLength 0 of the httpCaptureCookies`)
		ingressConfig = fmt.Sprintf(`ingress:
    accessLogging:
        httpCaptureCookies:
            - matchType: Exact
		...
		compat_otp.By(`5.0: Debug node and try to configure the name of the httpCaptureCookies with specail characters including space, # and so on`)
		ingressConfig = fmt.Sprintf(`ingress:
    accessLogging:
        httpCaptureCookies:
            - matchType: Exact
		...
		output = appendInvalidIngressConfigToConfigYaml(oc, e2eTestNamespace, caseID, nodeName, ingressConfig)
		compat_otp.By("output is :" + output)
		o.Expect(output).To(o.ContainSubstring("contains invalid characters"))

		compat_otp.By(`6.0: Debug node and try to configure invalid maxLength -1 of the httpCaptureHeaders`)
		ingressConfig = fmt.Sprintf(`ingress:
    accessLogging:
        httpCaptureHeaders:
            request:
		...
		compat_otp.By(`7.0: Debug node and try to configure status with invalid Enable(not Enabled)`)
		ingressConfig = fmt.Sprintf(`ingress:
    accessLogging:
        httpCaptureHeaders:
            request:
		...
		// OCP-82042 disable logging function
		compat_otp.By("8.0: Debug node and configure capture and log http cookies with specific prefixes via httpCaptureCookies option, but without enable the status")
		ingressConfig = fmt.Sprintf(`
ingress:
    accessLogging:
        httpCaptureCookies:
...
`)
		...
		compat_otp.By("9.0: Check the configuration of haproxy which should not contain the logging httpCaptureCookies")
		routerpod := getOneNewRouterPodFromRollingUpdate(oc, "default")
