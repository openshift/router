...
var _ = g.Describe("[sig-network-edge][OTP] Network_Edge Component_ExtDNS", func() {
	defer g.GinkgoRecover()

	var (
		oc                = compat_otp.NewCLI("externaldns", compat_otp.KubeConfigPath())
	...
	g.BeforeEach(func() {
		...
		compat_otp.By("Deploy External DNS konflux FBC")
		createExternalDNSCatalogSource(oc)
	})
	...
	g.It("ConnectedOnly-ARO-Author:hongli-High-48139-Support External DNS on Azure DNS provider", func() {
		...
		compat_otp.By("Ensure the case is runnable on the cluster")
		...
		compat_otp.By("Create CR ExternalDNS sample-azure-svc with invalid zone ID")
		...
		compat_otp.By("Patch externaldns with valid privateZone ID and wait until new operand pod ready")
		...
		compat_otp.AssertWaitPollNoErr(err, fmt.Sprintf("resource %v does not disapper", "pod/"+operandPod[0]))
		ensurePodWithLabelReady(oc, operatorNamespace, operandLabel)
		ensureLogsContainString(oc, operatorNamespace, operandLabel, "Found 1 Azure Private DNS zone")

		compat_otp.By("Add label to canary route, ensure ExternalDNS added the record")
		...
		compat_otp.By("Remove label from the canary route, ensure ExternalDNS deleted the record")
	...
	g.It("Author:hongli-ConnectedOnly-High-48140-Support External DNS on GCP DNS provider", func() {
		...
		compat_otp.By("Create CR ExternalDNS sample-gcp-svc and ensure operand pod is ready")
		ensurePodWithLabelReady(oc, operatorNamespace, operatorLabel)
		defer oc.AsAdmin().WithoutNamespace().Run("delete").Args("externaldns", crName).Output()
		time.Sleep(3 * time.Second)
		...
		compat_otp.AssertWaitPollNoErr(waitErr, fmt.Sprintf("resource %v does not disapper", "pod/"+operandPod[0]))
		ensurePodWithLabelReady(oc, operatorNamespace, operandLabel)
		ensureLogsContainString(oc, operatorNamespace, operandLabel, recordsReadyLog)

		compat_otp.By("Add label to canary service, ensure ExternalDNS added the record")
		...
		compat_otp.By("Remove label from the canary service, ensure ExternalDNS deleted the record")
	...
	g.It("Author:mjoseph-ConnectedOnly-ROSA-OSD_CCS-Critical-68826-External DNS support for preexisting Route53 for Shared VPC clusters", func() {
		...
		compat_otp.By("2. Create External DNS Operator in the cluster")
		createExternalDNSOperator(oc)
		ensurePodWithLabelReady(oc, operatorNamespace, operatorLabel)

		compat_otp.By("3. Create CR ExternalDNS sample-aws-sharedvpc-rt and ensure operand pod is ready")
		defer oc.AsAdmin().WithoutNamespace().Run("delete").Args("externaldns", crName).Output()
		...
		compat_otp.By("4. Add label to canary route, ensure ExternalDNS added the record")
		defer oc.AsAdmin().WithoutNamespace().Run("label").Args("-n", routeNamespace, "route", routeName, delLabel, "--overwrite").Output()
		_, err = oc.AsAdmin().WithoutNamespace().Run("label").Args("-n", routeNamespace, "route", routeName, addLabel).Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		compat_otp.By("5. Remove label from the canary route, ensure ExternalDNS deleted the record")
		_, err = oc.AsAdmin().WithoutNamespace().Run("label").Args("-n", routeNamespace, "route", routeName, delLabel, "--overwrite").Output()
		o.Expect(err).NotTo(o.HaveOccurred())
...
var _ = g.Describe("[sig-network-edge][OTP] Network_Edge Component_ExtDNS on STS", func() {
	defer g.GinkgoRecover()

	var (
		oc                = compat_otp.NewCLI("externaldnssts", compat_otp.KubeConfigPath())
		operatorNamespace = "external-dns-operator"
		operatorLabel     = "name=external-dns-operator"
		operandLabelKey   = "app.kubernetes.io/instance="
		addLabel          = "external-dns.mydomain.org/publish=yes"
	...
	g.It("Author:mjoseph-ConnectedOnly-ROSA-OSD_CCS-High-74949-ExternalDNS operand support on STS cluster", func() {
		...
		compat_otp.By("1. Ensure the case is runnable on the cluster")
		compat_otp.SkipIfPlatformTypeNot(oc, "AWS")
		// Skip in Gov cluster
		...
		compat_otp.By("2. Create the External DNS Operator")
		createExternalDNSOperator(oc)
		ensurePodWithLabelReady(oc, operatorNamespace, operatorLabel)

		compat_otp.By("3. Prepare the STS related credentials like role, policy and secret for the cluster")
		defer clearUpExDnsStsCluster(oc, "74949")
		prepareStsCredForCluster(oc, "74949")

		compat_otp.By("4. Create STS ExternalDNS CR `sample-aws-sts-rt` and ensure operand pod is ready")
		baseDomain := getBaseDomain(oc)
		...
		compat_otp.By("5. Add label to canary route, ensure ExternalDNS added the record")
		defer oc.AsAdmin().WithoutNamespace().Run("label").Args("-n", routeNamespace, "route", routeName, delLabel, "--overwrite").Output()
		_, err = oc.AsAdmin().WithoutNamespace().Run("label").Args("-n", routeNamespace, "route", routeName, addLabel).Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		compat_otp.By("6. Remove label from the canary route, ensure ExternalDNS deleted the record")
		_, err = oc.AsAdmin().WithoutNamespace().Run("label").Args("-n", routeNamespace, "route", routeName, delLabel, "--overwrite").Output()
		o.Expect(err).NotTo(o.HaveOccurred())
...
var _ = g.Describe("[sig-network-edge][OTP][Level0] Network_Edge Component_ExtDNS", func() {
	defer g.GinkgoRecover()
	...
	g.It("Author:hongli-ConnectedOnly-ROSA-OSD_CCS-High-48138-Support External DNS on AWS platform", func() {
		var (
			buildPruningBaseDir = testdata.FixturePath("router", "extdns")
		...
		compat_otp.By("Create CR ExternalDNS sample-aws-rt and ensure operand pod is ready")
		ensurePodWithLabelReady(oc, operatorNamespace, operatorLabel)
		defer oc.AsAdmin().WithoutNamespace().Run("delete").Args("externaldns", crName).Output()
		// To avoid connection refused flake error, as the controller CR creation needs extra prepare time after the operator pod is ready
		time.Sleep(3 * time.Second)
