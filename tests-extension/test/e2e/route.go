...
var _ = g.Describe("[OTP][sig-network-edge] Network_Edge Component_Router", func() {
	...
	g.It("Author:mjoseph-ROSA-OSD_CCS-ARO-High-88075-UnmanagedRoutes metric should filter ingress by both name and namespace [Serial]", func() {
		var (
			...
			routerFixtureDir      = testdata.FixturePath("router")
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		ensurePodWithLabelReady(oc, unmanagedNamespace, "name=web-server-deploy")
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		createResourceFromFile(oc, ns, ingressTemplate)
		...
		managedIngressName := getByJsonPath(oc, ns, "ingress/"+ingressName, "{..metadata.name}")
		o.Expect(managedIngressName).To(o.Equal(ingressName))
		unmanagedIngressName := getByJsonPath(oc, unmanagedNamespace, "ingress/"+ingressName, "{..metadata.name}")
		...
		pollErr := wait.Poll(60*time.Second, 30*time.Second, func() (bool, error) {
			metric, err := getPrometheusMetrics(oc, token, query)
			if err != nil {
				return false, nil
			}
