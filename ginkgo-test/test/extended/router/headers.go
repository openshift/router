...
`)
		...
		forwardedCfg1 := readHaproxyConfig(oc, routerpod, backend1Start, "-A10", "X-Forwarded-Host")
		o.Expect(forwardedCfg1).To(o.ContainSubstring(`http-request set-header X-Forwarded-Host %[req.hdr(host)]`))
...
`)
		...
		orwardedCfg := readHaproxyConfig(oc, routerpod, backendStart, "-A10", "option forwardfor")
		o.Expect(orwardedCfg).To(o.ContainSubstring("if-none"))
