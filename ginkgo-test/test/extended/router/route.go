...
var _ = g.Describe("[sig-network-edge] Network_Edge Component_Router", func() {
	...
	var oc = compat_otp.NewCLI("routes", compat_otp.KubeConfigPath())
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-Medium-10207-NetworkEdge Should use the same cookies for secure and insecure access when insecureEdgeTerminationPolicy set to allow for edge/reencrypt route", func() {
		...
		compat_otp.By("2.0: Deploy a project with a client pod, two server pods and the service")
		...
		err = oc.AsAdmin().WithoutNamespace().Run("exec").Args("-n", project1, clientPodName, "--", "mkdir", podFileDir).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		ensureRouteIsAdmittedByIngressController(oc, project1, "route-edge10207", "default")
		...
		curlCmd = []string{"-n", project1, clientPodName, "--", "curl", "https://" + routehost, "-ks", "-c", podFileDir + "/cookie-10207", "--resolve", toDst, "--connect-timeout", "10"}
		...
		err = oc.AsAdmin().WithoutNamespace().Run("cp").Args("-n", project1, clientPodName+":"+podFileDir+"/cookie-10207", fileDir+"/cookie-10207").Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		curlCmdWithCookie := []string{"-n", project1, clientPodName, "--", "curl", "https://" + routehost, "-ks", "-b", podFileDir + "/cookie-10207", "--resolve", toDst, "--connect-timeout", "10"}
		...
		curlCmdWithCookie = []string{"-n", project1, clientPodName, "--", "curl", "http://" + routehost, "-ksSL", "-b", podFileDir + "/cookie-10207", "--resolve", toDst, "--resolve", toDst2, "--connect-timeout", "10"}
		_, result = repeatCmdOnClient(oc, curlCmdWithCookie, expectOutput, 120, 6)
		o.Expect(result[1]).To(o.Equal(6))
		...
		ensureRouteIsAdmittedByIngressController(oc, project1, "route-reen10207", "default")

		compat_otp.By("9.0: Curl the route and generate a cookie file")
		curlCmdWithCookie = []string{"-n", project1, clientPodName, "--", "curl", "http://" + reenhost, "-ks", "-c", podFileDir + "/reen-cookie", "--resolve", toDst, "--resolve", toDst2, "--connect-timeout", "10"}
		...
		// access the cookie file and confirm that the output contains false and false
		err = oc.AsAdmin().WithoutNamespace().Run("cp").Args("-n", project1, clientPodName+":"+podFileDir+"/reen-cookie", fileDir+"/reen-cookie").Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-Low-10943-NetworkEdge Set invalid timeout server for route", func() {
		...
		compat_otp.By("1.0: Deploy a project with single pod and the service")
		project1 := oc.Namespace()
		...
		routerpod := getOneRouterPodNameByIC(oc, "default")
		ensureHaproxyBlockConfigContains(oc, routerpod, project1, []string{unSecSvcName, "timeout server  -2s"})
	})
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-NonHyperShiftHOST-Critical-11036-NetworkEdge Set insecureEdgeTerminationPolicy to Redirect for passthrough/edge/reencrypt route", func() {
		...
		compat_otp.By("1.0: Deploy a project with single pod, service and a passthrough/edge/reencrypt route")
		project1 := oc.Namespace()
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-Medium-11067-NetworkEdge oc help information should contain option wildcard-policy", func() {
		...
		compat_otp.By("1.0: Deploy a project with single pod, service")
		project1 := oc.Namespace()
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-Critical-11130-NetworkEdge Enable/Disable haproxy cookies based sticky session for edge termination routes", func() {
		...
		ensureRouteIsAdmittedByIngressController(oc, project1, "route-edge11130", "default")
	...
	g.It("Author:mjoseph-ROSA-OSD_CCS-ARO-Critical-11619-Limit the number of TCP connection per IP in specified time period", func() {
		...
		compat_otp.By("2. Create a passthrough route in the project")
		...
		routehost := getRouteHost(oc, project1, "mypass")
		...
		compat_otp.By("6. Expose a service in the project")
		...
		routehost = getRouteHost(oc, project1, "service-unsecure")
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-Critical-11635-NetworkEdge Set timeout server for passthough route", func() {
		...
		ensureRouteIsAdmittedByIngressController(oc, project1, routeName, "default")
		...
		routerpod := getOneRouterPodNameByIC(oc, "default")
		ensureHaproxyBlockConfigContains(oc, routerpod, project1, []string{routeName, "timeout tunnel  3s"})
	})
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-Medium-11728-haproxy hash based sticky session for tcp mode passthrough routes", func() {
		...
		compat_otp.SkipIfPlatformType(oc, "IBMCloud")
		...
		compat_otp.By("3.0: Create a passthrough route")
		...
		compat_otp.By("4.0: Check the passthrough route configuration in haproxy")
		routerpod := getOneRouterPodNameByIC(oc, "default")
		...
		compat_otp.By("5.0: Curl the passthrough route, and save the output")
		...
		compat_otp.By("6.0: Curl the passthrough route for 6 times, all are forwarded to the expected server")
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-High-11982-NetworkEdge Set timeout server for http route", func() {
		...
		compat_otp.By("5.0: Check HAProxy file for timeout tunnel")
		routerpod := getOneRouterPodNameByIC(oc, "default")
		ensureHaproxyBlockConfigContains(oc, routerpod, project1, []string{routeName, "timeout server  2s"})
	})
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-Critical-12091-haproxy config information should be clean when changing the service to another route", func() {
		...
		compat_otp.By("1.0: Deploy two sets of web-server and services")
		ns := oc.Namespace()
		...
		compat_otp.By("2.0: Create a unsecure route")
		...
		compat_otp.By("3.0: Add the balance=roundrobin annotation to the route, then check it in haproxy")
		routerpod := getOneRouterPodNameByIC(oc, "default")
		...
		compat_otp.By("4.0: Curl the http route, make sure the first server is hit")
		curlCmd := fmt.Sprintf(`curl http://%s -s --connect-timeout 10`, routehost)
		expectOutput := []string{"Hello-OpenShift " + pod1Name + " http-8080"}
		repeatCmdOnClient(oc, curlCmd, expectOutput, 60, 1)

		compat_otp.By("5.0: Patch the http route with spec to another service")
		...
		compat_otp.By("6.0: Check the route configuration in haproxy, make sure the first service disappeared and the second service present")
		ensureHaproxyBlockConfigNotMatchRegexp(oc, routerpod, backendStart, []string{webServerDeploy1.svcUnsecureName})
		...
		compat_otp.By("7.0: Curl the route for 10 times, all are forwarded to the second server")
	...
	g.It("Author:mjoseph-High-12506-reencrypt route with no cert if a router is configured with a default wildcard cert", func() {
		...
		waitForOutputContains(oc, oc.Namespace(), "route/12506-no-cert", "{.spec.tls}", "destinationCACertificate")

		compat_otp.By("4. Check the router pod and ensure the routes are loaded in haproxy.config of default controller")
		ensureHaproxyBlockConfigContains(oc, defaultContPod, project1, []string{"backend be_secure:" + project1 + ":12506-no-cert"})
	...
	g.It("Author:mjoseph-Critical-12564-The path specified in route can work well for reencrypt terminated", func() {
		...
		compat_otp.By("3. Confirm whether the destination certificate is present")
		waitForOutputContains(oc, oc.Namespace(), "route/12564-reencrypt", "{.spec.tls}", "destinationCACertificate")

		compat_otp.By("4. Check the router pod and ensure the routes are loaded in haproxy.config of default controller")
		ensureHaproxyBlockConfigContains(oc, defaultContPod, project1, []string{"backend be_secure:" + project1 + ":12564-reencrypt"})

		compat_otp.By("5. Check the reachability of the  in the specified path")
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-NonHyperShiftHOST-Critical-13753-NetworkEdge Check the cookie if using secure mode when insecureEdgeTerminationPolicy to Redirect for edge/reencrypt route", func() {
		...
		compat_otp.By("2.0: Deploy a project with two server pods and the service")
		project1 := oc.Namespace()
		srvPodList := createResourceFromWebServer(oc, project1, testPodSvc, srvrcInfo)
		...
		ensureRouteIsAdmittedByIngressController(oc, project1, "edge-route", "default")
		output, err := oc.Run("get").Args("route/edge-route", "-n", project1, "-o=jsonpath={.spec.tls}").Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(output).To(o.ContainSubstring(`"insecureEdgeTerminationPolicy":"Redirect"`))

		createRoute(oc, project1, "reencrypt", "reen-route", SvcName, []string{"--insecure-policy=Redirect"})
		ensureRouteIsAdmittedByIngressController(oc, project1, "reen-route", "default")
		output, err = oc.Run("get").Args("route/reen-route", "-n", project1, "-o=jsonpath={.spec.tls}").Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(output).To(o.ContainSubstring(`"insecureEdgeTerminationPolicy":"Redirect"`))
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-NonHyperShiftHOST-Critical-13839-NetworkEdge Set insecureEdgeTerminationPolicy to Allow for reencrypt/edge route", func() {
		...
		compat_otp.By("1.0: Deploy a project with single pod, service and reencrypt and edge route")
		project1 := oc.Namespace()
		srvPodList := createResourceFromWebServer(oc, project1, testPodSvc, "web-server-deploy")
		output, err := oc.Run("get").Args("service").Output()
		o.Expect(err).NotTo(o.HaveOccurred())
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-NonHyperShiftHOST-Critical-14678-NetworkEdge Only the host in whitelist could access unsecure/edge/reencrypt/passthrough routes", func() {
		...
		compat_otp.By("1.0: Deploy a project with Pod and Services")
		project1 := oc.Namespace()
		routerpod := getOneRouterPodNameByIC(oc, "default")
		...
		ensureRouteIsAdmittedByIngressController(oc, project1, "route-unsecure", "default")
		...
		ensureRouteIsAdmittedByIngressController(oc, project1, "route-passthrough", "default")
		...
		ensureHaproxyBlockConfigContains(oc, routerpod, project1+":"+unsecureRoute, []string{"acl allowlist src 5.6.7.8"})
		ensureHaproxyBlockConfigContains(oc, routerpod, project1+":"+edgeRoute, []string{"acl allowlist src 5.6.7.8"})
		ensureHaproxyBlockConfigContains(oc, routerpod, project1+":"+passthroughRoute, []string{"acl allowlist src 5.6.7.8"})
		ensureHaproxyBlockConfigContains(oc, routerpod, project1+":"+reenRoute, []string{"acl allowlist src 5.6.7.8"})
	})
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-Low-14680-NetworkEdge Add invalid value in annotation whitelist to route", func() {
		...
		compat_otp.By("7.0: Check HaProxy if the IP in the whitelist annotation exists")
		ensureHaproxyBlockConfigContains(oc, routerpod, project1+":"+unsecureRoute, []string{"acl allowlist src 0.0.0.0/0"})
	})
	...
	g.It("Author:mjoseph-ROSA-OSD_CCS-ARO-High-15028-router can do a case-insensitive match of a hostname for unsecure/edge/passthrough/reencrypt route", func() {
		...
		compat_otp.By("2. Create a unsecure route and ensure the routes are loaded in haproxy.config of default controlle")
		createRoute(oc, project1, "http", "15028-http", UnsecureSvcName, []string{})
		ensureRouteIsAdmittedByIngressController(oc, project1, "15028-http", "default")
		httpHostCapital := "15028-HTTP-" + project1 + ".apps." + baseDomain

		compat_otp.By("3. Create a edge route and ensure the routes are loaded in haproxy.config of default controlle")
		createRoute(oc, project1, "edge", "15028-edge", UnsecureSvcName, []string{})
		ensureRouteIsAdmittedByIngressController(oc, project1, "15028-edge", "default")
		edgeHostCapital := "15028-EDGE-" + project1 + ".apps." + baseDomain

		compat_otp.By("4. Create a passthrough route and ensure the routes are loaded in haproxy.config of default controlle")
		createRoute(oc, project1, "passthrough", "15028-pass", SecureSvcName, []string{})
		ensureRouteIsAdmittedByIngressController(oc, project1, "15028-pass", "default")
		passHostCapital := "15028-PASS-" + project1 + ".apps." + baseDomain

		compat_otp.By("5. Create a reen route and ensure the routes are loaded in haproxy.config of default controlle")
		createRoute(oc, project1, "reencrypt", "15028-reen", SecureSvcName, []string{"--dest-ca-cert=" + caCert})
		ensureRouteIsAdmittedByIngressController(oc, project1, "15028-reen", "default")
	...
	g.It("Author:mjoseph-ROSA-OSD_CCS-ARO-High-15113-Harden haproxy to prevent the PROXY header from being passed for unsecure/edge route", func() {
		...
		compat_otp.By("2. Create a unsecure route")
		createRoute(oc, project1, "http", "15113-http", UnsecureSvcName, []string{})
		ensureRouteIsAdmittedByIngressController(oc, project1, "15113-http", "default")
		getRoutes(oc, oc.Namespace())
		...
		ensureRouteIsAdmittedByIngressController(oc, project1, "15113-edge", "default")
		getRoutes(oc, oc.Namespace())
		edgeHost := "15113-edge-" + project1 + ".apps." + baseDomain

		// OCP-10762: Check the header forward format
		compat_otp.By("4. Access the route and check the header forward format")
		...
		compat_otp.By("5. Access the route with 'proxy' header and confirm the proxy is not carried with it")
		...
		compat_otp.By("6. Access the route with 'proxy' header and confirm the proxy is not carried with it")
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-Critical-15873-NetworkEdge can set cookie name for edge/reen routes by annotation", func() {
		...
		ensureRouteIsAdmittedByIngressController(oc, project1, "route-edge15873", "default")
		...
		ensureRouteIsAdmittedByIngressController(oc, project1, "route-reen15873", "default")
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-Medium-16732-NetworkEdge Check haproxy.config when overwriting 'timeout server' which was already specified", func() {
		...
		routerpod := getOneRouterPodNameByIC(oc, "default")
		ensureHaproxyBlockConfigContains(oc, routerpod, project1+":"+routeName, []string{"timeout server  5s"})
		...
		compat_otp.By("6.0: Check HAProxy file again for timeout server")
		ensureHaproxyBlockConfigContains(oc, routerpod, project1, []string{project1 + ":" + routeName})
	})
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-Critical-18482-NetworkEdge limits backend pod max concurrent connections for unsecure, edge, reen, passthrough route", func() {
		...
		compat_otp.By("1.0: Deploy a project with single pod and the services")
		project1 := oc.Namespace()
		createResourceFromFile(oc, project1, testPodSvc)
		ensurePodWithLabelReady(oc, project1, "name=web-server-deploy")
		output, err := oc.Run("get").Args("service").Output()
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-Medium-18490-NetworkEdge limits multiple backend pods max concurrent connections", func() {
		...
		compat_otp.By("1.0: Deploy a project with single pod and its services")
		project1 := oc.Namespace()
		createResourceFromFile(oc, project1, testPodSvc)
		ensurePodWithLabelReady(oc, project1, "name=web-server-deploy")
		output, err := oc.Run("get").Args("service").Output()
		...
		waitForOutputEquals(oc, project1, "deployment/web-server-deploy", "{.status.availableReplicas}", "2")
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-High-34106-NetworkEdge Routes annotated with 'haproxy.router.openshift.io/rewrite-target=/path' will replace and rewrite http request with specified '/path'", func() {
		...
		createRoute(oc, project1, "http", unSecSvcName, unSecSvcName, []string{})
		ensureRouteIsAdmittedByIngressController(oc, project1, "service-unsecure", "default")
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-Critical-38671-NetworkEdge 'haproxy.router.openshift.io/timeout-tunnel' annotation gets applied alongside 'haproxy.router.openshift.io/timeout' for clear/edge/reencrypt routes", func() {
		...
		compat_otp.By("1.0: Deploy a project with single pod and 3 services")
		project1 := oc.Namespace()
		createResourceFromFile(oc, project1, testPodSvc)
		ensurePodWithLabelReady(oc, project1, "name="+srvrcInfo)
		output, err := oc.Run("get").Args("service").Output()
		...
		routerpod := getOneRouterPodNameByIC(oc, "default")
		ensureHaproxyBlockConfigContains(oc, routerpod, project1+":"+routeName, []string{"timeout server  15s"})
		ensureHaproxyBlockConfigContains(oc, routerpod, project1+":edge-route", []string{"timeout server  15s"})
		ensureHaproxyBlockConfigContains(oc, routerpod, project1+":reen-route", []string{"timeout server  15s"})
		...
		ensureHaproxyBlockConfigContains(oc, routerpod, project1+":"+routeName, []string{"timeout tunnel  5s"})
		ensureHaproxyBlockConfigContains(oc, routerpod, project1+":"+routeName, []string{"timeout tunnel  5s"})
		ensureHaproxyBlockConfigContains(oc, routerpod, project1+":"+routeName, []string{"timeout tunnel  5s"})
	})
	...
	g.It("Author:aiyengar-ROSA-OSD_CCS-ARO-Medium-42230-route can be configured to whitelist more than 61 ips/CIDRs", func() {
		...
		//backendName is the leading context of the route
		ensureHaproxyBlockConfigContains(oc, podName, backendName, []string{`acl allowlist src -f /var/lib/haproxy/router/allowlists/` + oc.Namespace() + `:service-unsecure.txt`, `tcp-request content reject if !allowlist`})
	})
	...
	g.It("Author:mjoseph-ROSA-OSD_CCS-ARO-High-45399-ingress controller continue to function normally with unexpected high timeout value", func() {
		...
		podName := getOneRouterPodNameByIC(oc, "default")
		ensureHaproxyBlockConfigContains(oc, podName, oc.Namespace(), []string{"timeout server  2147483647ms"})
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-NonPreRelease-PreChkUpgrade-High-45955-Unidling a route work without user intervention", func() {
		...
		compat_otp.By("1.0: Create a new project and a web-server application")
		...
		compat_otp.By("2.0: Confirm that the service exists and expose it")
		...
		compat_otp.By("3.0: Access the exposed route")
		routehost := getRouteHost(oc, ns, "service-unsecure")
		curlCmd := fmt.Sprintf(`curl http://%s --connect-timeout 10`, routehost)
		repeatCmdOnClient(oc, curlCmd, "Hello-OpenShift", 120, 1)

		compat_otp.By("4.0: Idle the service and wait for the pod resource to dissapear")
		...
		compat_otp.By("5.0: Confirm the service annotation is present")
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-NonPreRelease-PstChkUpgrade-High-45955-Unidling a route work without user intervention", func() {
		...
		compat_otp.By("1.0: Confirm that the service is still available with the annotation")
		_, err := oc.AsAdmin().WithoutNamespace().Run("get").Args("-n", ns, "service", "service-unsecure").Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		findAnnotation := getAnnotation(oc, ns, "service", "service-unsecure")
		o.Expect(findAnnotation).To(o.ContainSubstring(`idling.alpha.openshift.io/unidle-targets":"[{\"kind\":\"Deployment\",\"name\":\"web-server-deploy\",\"group\":\"apps\",\"replicas\":1}]`))

		compat_otp.By("2.0: Access the route to unidle the service")
		routehost := getRouteHost(oc, ns, "service-unsecure")
		curlCmd := fmt.Sprintf(`curl http://%s --connect-timeout 10`, routehost)
		repeatCmdOnClient(oc, curlCmd, "Hello-OpenShift", 120, 1)

		compat_otp.By("3.0: Check the service to see if the idle annotation is removed")
		findAnnotation = getAnnotation(oc, ns, "service", "service-unsecure")
	...
	g.It("Author:mjoseph-ROSA-OSD_CCS-ARO-High-49802-HTTPS redirect happens even if there is a more specific http-only", func() {
		...
		statsOut, err := compat_otp.RemoteShPod(oc, project1, podName[0], "sh", "-c", curlCmd)
		o.Expect(err).NotTo(o.HaveOccurred())
	...
	g.It("Author:hongli-ROSA-OSD_CCS-ARO-High-73771-router can load secret", func() {
		...
		compat_otp.By("Create edge/passthrough/reencrypt routes and all should be reachable")
		extraParas := []string{}
		...
		compat_otp.By("should be failed if patch the edge route without required role and secret")
		...
		compat_otp.By("create required role/rolebinding and secret")
		...
		compat_otp.By("patch the edge and reen route, but only edge route should be reachable")
		...
		compat_otp.By("renew the server certificate with multi SAN and refresh the secret")
		...
		compat_otp.By("with the updated secret, both edge and reen route should be reachable")
		waitForOutsideCurlContains("https://"+edgeRouteHost, curlOptions, "Hello-OpenShift")
		waitForOutsideCurlContains("https://"+reenRouteHost, curlOptions, "Hello-OpenShift")

		compat_otp.By("should failed to patch passthrough route with externalCertificate")
		...
		compat_otp.By("edge route reports error after deleting the referenced secret")
		err = oc.Run("delete").Args("-n", project1, "secret", "mytls").Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
		waitForOutputContains(oc, project1, "route/myedge", `{.status.ingress[?(@.routerName=="default")].conditions[*]}`, "ExternalCertificateValidationFailed")

		// https://issues.redhat.com/browse/OCPBUGS-33958 (4.19+)
		compat_otp.By("edge and reen route should be recovered after recreating the referenced secret")
		err = oc.WithoutNamespace().Run("apply").Args("-f", newSecretYaml).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
		ensureRouteIsAdmittedByIngressController(oc, project1, "myedge", "default")
		waitForOutsideCurlContains("https://"+edgeRouteHost, curlOptions, "Hello-OpenShift")
		waitForOutsideCurlContains("https://"+reenRouteHost, curlOptions, "Hello-OpenShift")
	})
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-Critical-77080-NetworkEdge Only host in allowlist can access unsecure/edge/reencrypt/passthrough routes", func() {
		...
		ensureHaproxyBlockConfigContains(oc, routerpod, project1+":"+unsecureRoute, []string{"acl allowlist src 1050::5:600:300c:326b", "tcp-request content reject if !allowlist"})
		ensureHaproxyBlockConfigContains(oc, routerpod, project1+":"+edgeRoute, []string{"acl allowlist src 8.8.8.8", "tcp-request content reject if !allowlist"})
		ensureHaproxyBlockConfigContains(oc, routerpod, project1+":"+passthroughRoute, []string{"acl allowlist src 1050::5:600:300c:326b", "tcp-request content reject if !allowlist"})
		ensureHaproxyBlockConfigContains(oc, routerpod, project1+":"+reenRoute, []string{"acl allowlist src 8.8.4.4", "tcp-request content reject if !allowlist"})
	})
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-Critical-77082-NetworkEdge Route gives allowlist precedence when whitelist and allowlist annotations are both present", func() {
		...
		ensureHaproxyBlockConfigContains(oc, routerpod, project1+":"+unsecureRoute, []string{"acl allowlist src", "tcp-request content reject if !allowlist"})
	})
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-High-77091-NetworkEdge Route does not enable allowlist with than 61 CIDRs and if invalid IP annotation is given", func() {
		...
		ensureHaproxyBlockConfigNotContains(oc, routerpod, project1+":"+edgeRoute, []string{"acl allowlist src", "tcp-request content reject if !allowlist"})
		...
		ensureHaproxyBlockConfigContains(oc, routerpod, project1+":"+unsecureRoute, []string{"acl allowlist src 192.168.0.0/24", "tcp-request content reject if !allowlist"})
		...
		ensureHaproxyBlockConfigContains(oc, routerpod, project1+":"+unsecureRoute, []string{`acl allowlist src -f /var/lib/haproxy/router/allowlists/` + project1 + ":" + unsecureRoute + ".txt", "tcp-request content reject if !allowlist"})
		ensureHaproxyBlockConfigNotContains(oc, routerpod, project1+":"+unsecureRoute, []string{"acl allowlist src 192.168.0.0/24"})
	})
})
