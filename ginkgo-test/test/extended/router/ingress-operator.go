...
var _ = g.Describe("[sig-network-edge] Network_Edge Component_Router", func() {
	...
	var oc = compat_otp.NewCLI("ingress-operator", compat_otp.KubeConfigPath())
	...
	g.It("Author:hongli-ROSA-OSD_CCS-ARO-Critical-21873-The replicas of router deployment is controlled by ingresscontroller", func() {
		...
		compat_otp.By("Ensure NodePort service is created")
		...
		compat_otp.By("Scale the replicas to 0 and ensure router pod is deleted")
		...
		compat_otp.AssertWaitPollNoErr(err, fmt.Sprintf("The router pod %v does not disapper", routerpod))

		compat_otp.By("Scale the replicas to 1 and ensure deployment is updated")
	...
	g.It("Author:hongli-NonHyperShiftHOST-ROSA-OSD_CCS-ARO-Medium-22633-The nodeSelector and tolerations of router deployment are controlled by ingresscontrolle", func() {
		...
		compat_otp.By("Check the default nodeSelector and tolerations")
		...
		compat_otp.By("Update the ingresscontroller nodeSelector and tolerations to deploy router pod to control-plane node")
		...
		compat_otp.By("Ensure the nodeSelector and tolerations of router deployment is updated")
		nodeSelector = getByJsonPath(oc, "openshift-ingress", "deployment/router-"+ingctrl.name, "{.spec.template.spec.nodeSelector}")
	...
	g.It("Author:mjoseph-ROSA-OSD_CCS-ARO-Critical-22636-The namespaceSelector of router is controlled by ingresscontroller", func() {
		...
		compat_otp.By("1. Create one custom ingresscontroller")
		...
		compat_otp.By("2. Create a server pod and expose an unsecure service")
		...
		waitForOutputEquals(oc, project1, "route/"+srvName, "{.spec.host}", routehost)

		compat_otp.By("3. Label the namespace to 'namespace=router-test'")
		err = oc.AsAdmin().WithoutNamespace().Run("label").Args("namespace", project1, "namespace=router-test").Execute()
		o.Expect(err).NotTo(o.HaveOccurred())

		compat_otp.By("4. Patch the custom ingresscontroller with the namespaceSelector")
		...
		compat_otp.By("5. Check the haproxy config on the custom router pod to find the backend details of the " + project1 + " route")
		...
		compat_otp.By("6. Check the haproxy config on the custom router to confirm no backend details of other routes are present")
	...
	g.It("Author:mjoseph-ROSA-OSD_CCS-ARO-High-22637-The routeSelector of router is controlled by ingresscontroller", func() {
		...
		compat_otp.By("3. Label the route to 'route=router-test'")
		...
		compat_otp.By("5. Check the haproxy config on the custom router pod to find the backend details of the route for service-unsecure")
		checkoutput := readRouterPodData(oc, newCustContPod, "cat haproxy.config", "service-unsecure")
		o.Expect(checkoutput).To(o.ContainSubstring("backend be_http:" + project1 + ":service-unsecure"))
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-High-29207-the PROXY protocol is disabled in non-AWS platform", func() {
		...
		compat_otp.By("1.0 Get default ingress pod")
		routerpod := getOneRouterPodNameByIC(oc, "default")

		compat_otp.By("2.0 Check HaProxy for PROXY protocol")
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-Critical-30059-NetworkEdge Create an ingresscontroller that logs to a sidecar container", func() {
		...
		compat_otp.By("1. Create a sidecar-log ingresscontroller")
		baseDomain := getBaseDomain(oc)
		ingctrl.domain = ingctrl.name + "." + baseDomain
		defer ingctrl.delete(oc)
		ingctrl.create(oc)
		...
		compat_otp.By("2. Check if the a sidecar container with name 'logs' is deployed")
		...
		compat_otp.By("3. Create a client pod, a server pod and an unsecure service")
		ns := oc.Namespace()
		...
		compat_otp.By("4. Create an route")
		...
		compat_otp.By("5. Curl the route, and then check the router pod's logs which should contain the new http request")
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-Critical-30060-NetworkEdge Create an ingresscontroller that logs to external rsyslog instance", func() {
		...
		compat_otp.By("1. Create a syslog pod for the log receiver")
		ns := oc.Namespace()
		...
		compat_otp.By("2. Create a syslog ingresscontroller")
		...
		compat_otp.By("3. Check ROUTER_SYSLOG_ADDRESS env in a router pod")
		routerpod := getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)
		...
		compat_otp.By("4. Create a client pod, a server pod and an unsecure service")
		createResourceFromFile(oc, ns, clientPod)
		ensurePodWithLabelReady(oc, ns, clientPodLabel)
		...
		compat_otp.By("5. Create a route")
		routehost := srvName + "-" + ns + "." + ingctrl.domain
		...
		compat_otp.By("6. Curl the route, and then check the rsyslogd pod's logs which should contain the new http request")
		podIP := getPodv4Address(oc, routerpod, "openshift-ingress")
		toDst := routehost + ":80:" + podIP
		jsonPath := fmt.Sprintf(`{.items[?(@.metadata.generateName=="%s-")].metadata.name}`, srvName)
		epSliceName := getByJsonPath(oc, ns, "EndpointSlice", jsonPath)
	...
	// author: iamin@redhat.com
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-Medium-30066-Enable log-send-hostname in HAproxy configuration by default", func() {
		var (
			buildPruningBaseDir = compat_otp.FixturePath("testdata", "router")
			customTemp          = filepath.Join(buildPruningBaseDir, "ingresscontroller-sidecar.yaml")
			ingctrl             = ingressControllerDescription{
		...
		compat_otp.By("2. Verify presence of pod with label")
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-Medium-30264-ROUTER_SYSLOG_ADDRESS changes according to the logging configuration", func() {
		...
		compat_otp.By("2. check router deployment")
		err := oc.AsAdmin().WithoutNamespace().Run("get").Args("deployment", "-n", "openshift-ingress", "router-"+ingctrl.name).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())

		compat_otp.By("3. Verify presence of pod with label")
		ingPod := getOnePodNameByLabel(oc, "openshift-ingress", "ingresscontroller.operator.openshift.io/deployment-ingresscontroller="+ingctrl.name)
		o.Expect(ingPod).To(o.ContainSubstring(ingctrl.name))
		...
		compat_otp.By("4. check custom ingress pod for ROUTER_SYSLOG_ADDRESS")
		...
		compat_otp.By("5. Patch the ingress controller with a different address and port")
		...
		compat_otp.By("6. Wait for previous pod to be deleted")
		waitErr := wait.PollImmediate(2*time.Second, 300*time.Second, func() (bool, error) {
			output, _ := oc.AsAdmin().WithoutNamespace().Run("get").Args("pod", "-n", "openshift-ingress").Output()
			if strings.Contains(output, ingPod) {
				e2e.Logf("The Pod is still active..., retrying")
				return false, nil
			}
			e2e.Logf("The Pod has been deleted!")
			return true, nil
		})
		o.Expect(waitErr).NotTo(o.HaveOccurred(), "The previous ingress pod was never deleted")

		compat_otp.By("7. Retrieve new pod name with label")
		ingPod = getOnePodNameByLabel(oc, "openshift-ingress", "ingresscontroller.operator.openshift.io/deployment-ingresscontroller="+ingctrl.name)
		o.Expect(ingPod).To(o.ContainSubstring(ingctrl.name))

		compat_otp.By("8. check custom ingress pod for ROUTER_SYSLOG_ADDRESS")
		output, err = oc.AsAdmin().WithoutNamespace().Run("exec").Args("-n", "openshift-ingress", ingPod, "--", "bash", "-lc", cmd).Output()
		o.Expect(err).NotTo(o.HaveOccurred())
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-Critical-33763-ingresscontroller supports AWS NLB", func() {
		...
		compat_otp.By("Check the Platform type")
		compat_otp.SkipIfPlatformTypeNot(oc, "AWS")

		compat_otp.By("1. Create a nlb ingress controller")
		baseDomain := getBaseDomain(oc)
		ingctrl.domain = ingctrl.name + "." + baseDomain
		...
		compat_otp.By("3. check if LB service exists")
		...
		compat_otp.By("4. Check LB service annotations")
	...
	g.It("Author:mjoseph-NonHyperShiftHOST-High-38674-hard-stop-after annotation can be applied globally on all ingresscontroller [Disruptive]", func() {
		...
		compat_otp.By("Annotate the ingresses.config/cluster with ingress.operator.openshift.io/hard-stop-after globally")
		...
		compat_otp.AssertWaitPollNoErr(err, fmt.Sprintf("resource %v does not disapper", "pod/"+defaultRouterpod))
		ensureRouterDeployGenerationIs(oc, ingctrl.name, "2")

		compat_otp.By("Verify the annotation presence in the cluster gloabl config")
		...
		compat_otp.By("Check the env variable of the custom router pod to verify the hard stop duration is 30m")
		env := readRouterPodEnv(oc, newRouterpod, "ROUTER_HARD_STOP_AFTER")
		o.Expect(env).To(o.ContainSubstring(`30m`))

		compat_otp.By("Check the env variable of the default router pod to verify the hard stop duration is 30m")
		env1 := readRouterPodEnv(oc, newDefaultRouterpod, "ROUTER_HARD_STOP_AFTER")
		o.Expect(env1).To(o.ContainSubstring(`30m`))

		compat_otp.By("Annotate the ingresses.config/cluster with ingress.operator.openshift.io/hard-stop-after per ingresscontroller basis")
		...
		compat_otp.By("Verify the annotation presence in the ocp38674 controller config")
		...
		compat_otp.By("Check the haproxy config on the defualt router pod to verify the hard stop value is still 30m")
		...
		compat_otp.By("Check the haproxy config on the router pod to verify the hard stop value is changed to 45m")
	...
	g.It("Author:mjoseph-High-38675-hard-stop-after annotation can be applied on per ingresscontroller", func() {
		...
		compat_otp.By("1. Create two custom ingresscontrollers")
		baseDomain := getBaseDomain(oc)
		...
		compat_otp.By("2. Annotate the router of " + ingctrl38675one.name + " with 15m hardstop value")
		...
		compat_otp.By("3. Annotate the router of " + ingctrl38675two.name + " with 30m hardstop value")
		...
		compat_otp.By("4. Check the haproxy config on the " + ingctrl38675one.name + " router pod to verify the hard stop value is 15m")
		...
		compat_otp.By("5. Check the haproxy config on the " + ingctrl38675two.name + " router pod to verify the hard stop value is 30m")
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-NonPreRelease-PreChkUpgrade-High-38812-upgrade with router shards", func() {
		...
		compat_otp.By("1.0: Confirm that the ingress co is in normal status")
		...
		compat_otp.By("2.0: Create a new project and a web-server application")
		...
		compat_otp.By("3.0: Create an edge route and label the route")
		...
		compat_otp.By("4.0: Get default ingresscontroller information and create shard ingresscontroller")
		ingctrl.domain = ingctrl.name + "." + baseDomain
		ingctrl.create(oc)
		ensureRouterDeployGenerationIs(oc, ingctrl.name, "1")
		ensurePodWithLabelReady(oc, "openshift-ingress", "ingresscontroller.operator.openshift.io/deployment-ingresscontroller=shards")

		compat_otp.By("5.0: Ensure the route in the matched namespace is loaded")
		...
		compat_otp.By("6.0: Confirm that the ingress co is in normal status")
		waitForOutputEquals(oc, "default", "co/ingress", jsonPath, "TrueFalseFalse")
	})
	...
	g.It("Author:iamin-ROSA-OSD_CCS-ARO-NonPreRelease-PstChkUpgrade-High-38812-upgrade with router shards", func() {
		...
		compat_otp.By("2.0: Confirm that the custom ingress router pod is still available and get its IP")
		...
		compat_otp.By("3.0: Ensure that the route served by shards is still accessible after upgrade")
	...
	g.It("Author:hongli-Critical-42276-enable annotation traffic-policy.network.alpha.openshift.io/local-with-fallback on LB and nodePort service", func() {
		...
		compat_otp.By("check the annotation of nodeport service")
		...
		if !platforms[platformtype] {
			compat_otp.By("check the annotation of default LoadBalancer service if it is available")
	...
	g.It("Author:mjoseph-High-46287-ingresscontroller supports to update maxlength for syslog message", func() {
		...
		compat_otp.By("check the env variable of the router pod to verify the default log length")
		newrouterpod := getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)
		...
		compat_otp.By("check the haproxy config on the router pod to verify the default log length is enabled")
		...
		compat_otp.By("patch the existing custom ingress controller with minimum log length value")
		...
		compat_otp.By("check the env variable of the router pod to verify the minimum log length")
		...
		compat_otp.By("patch the existing custom ingress controller with maximum log length value")
		...
		compat_otp.By("check the env variable of the router pod to verify the maximum log length")
		newrouterpod = getOneNewRouterPodFromRollingUpdate(oc, "ocp46287")
	...
	g.It("Author:mjoseph-Low-46288-ingresscontroller should deny invalid maxlengh value for syslog message", func() {
		...
		compat_otp.By("patch the existing custom ingress controller with log length value less than minimum threshold")
		...
		compat_otp.By("patch the existing custom ingress controller with log length value more than maximum threshold")
	...
	g.It("Author:mjoseph-LEVEL0-Critical-51255-cluster-ingress-operator can set AWS ELB idle Timeout on per controller basis", func() {
		...
		compat_otp.By("Patch the new custom ingress controller with connectionIdleTimeout as 2m")
		...
		compat_otp.By("Check the connectionIdleTimeout value in the controller status")
	...
	g.It("Author:mjoseph-Medium-51256-cluster-ingress-operator does not accept negative value of AWS ELB idle Timeout option", func() {
		...
		compat_otp.By("Patch the new custom ingress controller with connectionIdleTimeout with a negative value")
		patchResourceAsAdmin(oc, ingctrl.namespace, "ingresscontroller/"+ingctrl.name, "{\"spec\":{\"endpointPublishingStrategy\":{\"loadBalancer\":{\"providerParameters\":{\"aws\":{\"classicLoadBalancer\":{\"connectionIdleTimeout\":\"-2m\"}}}}}}}")

		compat_otp.By("Check the LB service and ensure the annotation is not added")
		...
		compat_otp.By("Check the connectionIdleTimeout value is '0s' in the controller status")
	...
	g.It("Author:shudili-High-54868-Configurable dns Management for LoadBalancerService Ingress Controllers on AWS", func() {
		...
		compat_otp.By("Create two custom ingresscontrollers, one matches the cluster's base domain, the other doesn't")
		baseDomain := getBaseDomain(oc)
		...
		compat_otp.By("check the default dnsManagementPolicy value of ingress-controller1 matching the base domain, which should be Managed")
		...
		compat_otp.By("check ingress-controller1's status")
		...
		compat_otp.By("check the default dnsManagementPolicy value of dnsrecord ocp54868cus1, which should be Managed, too")
		...
		compat_otp.By("check dnsrecord ocp54868cus1's status")
		...
		compat_otp.By("patch custom ingress-controller1 with dnsManagementPolicy Unmanaged")
		defer func() {
		...
		compat_otp.By("check the dnsManagementPolicy value of ingress-controller1, which should be Unmanaged")
		...
		compat_otp.By("check the dnsManagementPolicy value of dnsrecord ocp54868cus1, which should be Unmanaged, too")
		...
		// compat_otp.By("check the default dnsManagementPolicy value of ingress-controller2 not matching the base domain, which should be Unmanaged")
		...
		compat_otp.By("check ingress-controller2's status")
		...
		// there was a bug OCPBUGS-2247 in the below test step
		// compat_otp.By("check the default dnsManagementPolicy value of dnsrecord ocp54868cus2, which should be Unmanaged, too")
		...
		compat_otp.By("check dnsrecord ocp54868cus2's status")
	...
	g.It("Author:shudili-Low-54995-Negative Test of Configurable dns Management for LoadBalancerService Ingress Controllers on AWS", func() {
		...
		compat_otp.By("try to patch the custom ingress-controller with dnsManagementPolicy unmanaged")
		...
		compat_otp.By("try to patch the custom ingress-controller with dnsManagementPolicy abc")
		...
		compat_otp.By("try to patch the custom ingress-controller with dnsManagementPolicy 123")
	...
	g.It("Author:mjoseph-LEVEL0-Critical-55223-Configuring list of IP address ranges using allowedSourceRanges in LoadBalancerService", func() {
		...
		compat_otp.By("Patch the custom ingress-controller with IP address ranges to which access to the load balancer should be restricted")
	...
	g.It("Author:mjoseph-High-55341-configuring list of IP address ranges using load-balancer-source-ranges annotation", func() {
		...
		compat_otp.By("Add the IP address ranges for an custom IngressController using annotation")
		...
		compat_otp.By("Verify the annotation presence")
		...
		compat_otp.By("Check the annotation value in the allowedSourceRanges in the controller status")
		...
		compat_otp.By("Patch the loadBalancerSourceRanges in the LB service")
		patchResourceAsAdmin(oc, "openshift-ingress", "svc/router-ocp55341", "{\"spec\":{\"loadBalancerSourceRanges\":[\"30.0.0.0/16\"]}}")

		compat_otp.By("Check the annotation value and sourcerange value in LB svc")
		findAnnotation = getAnnotation(oc, "openshift-ingress", "svc", "router-ocp55341")
		o.Expect(findAnnotation).To(o.ContainSubstring("service.beta.kubernetes.io/load-balancer-source-ranges"))
		o.Expect(findAnnotation).To(o.ContainSubstring("10.0.0.0/8"))
		waitForOutputContains(oc, "openshift-ingress", "svc/router-ocp55341", "{.spec.loadBalancerSourceRanges}", `30.0.0.0/16`)

		compat_otp.By("Check the controller status and confirm the sourcerange value's precedence over the annotation")
	...
	g.It("Author:mjoseph-Medium-55381-Configuring wrong data for allowedSourceRanges in LoadBalancerService", func() {
		...
		compat_otp.By("Patch the custom ingress-controller with only IP address")
		output, errCfg := patchResourceAsAdminAndGetLog(oc, ingctrl.namespace, ingctrlResource,
		...
		compat_otp.By("Patch the custom ingress-controller with a invalid IPv6 address")
		output, errCfg = patchResourceAsAdminAndGetLog(oc, ingctrl.namespace, ingctrlResource,
			"{\"spec\":{\"endpointPublishingStrategy\":{\"loadBalancer\":{\"allowedSourceRanges\":[\"3dee:ef5:/12\"]}}}}")
		o.Expect(errCfg).To(o.HaveOccurred())
		o.Expect(output).To(o.ContainSubstring("The IngressController \"ocp55381\" is invalid"))

		compat_otp.By("Patch the custom ingress-controller with IP address ranges")
		...
		compat_otp.By("Delete the allowedSourceRanges from custom controller")
		...
		compat_otp.By("Check the ingress operator status to confirm whether it is still in Progress")
		ensureClusterOperatorProgress(oc, "ingress")

		compat_otp.By("Patch the same loadBalancerSourceRanges value in the LB service to remove the Progressing from the ingress operator")
	...
	g.It("Author:shudili-Medium-56772-Ingress Controller does not set allowPrivilegeEscalation in the router deployment [Serial]", func() {
		...
		compat_otp.By("Create the custom-restricted SecurityContextConstraints")
		defer operateResourceFromFile(oc, "delete", "openshift-ingress", scc)
		operateResourceFromFile(oc, "create", "openshift-ingress", scc)

		compat_otp.By("check the allowPrivilegeEscalation in the router deployment, which should be true")
		...
		compat_otp.By("get router pods and then delete one router pod")
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		err = waitForResourceToDisappear(oc, "openshift-ingress", "pod/"+routerpod)
		compat_otp.AssertWaitPollNoErr(err, fmt.Sprintf("resource %v does not disapper", "pod/"+routerpod))

		compat_otp.By("get router pods again, and check if it is different with the previous router pod list")
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-NonPreRelease-Medium-60012-matchExpressions for routeSelector defined in an ingress-controller", func() {
		...
		compat_otp.By("Create 4 routes for the testing")
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		waitForOutputEquals(oc, project1, "route/unsrv-1", "{.metadata.name}", "unsrv-1")
		waitForOutputEquals(oc, project1, "route/unsrv-2", "{.metadata.name}", "unsrv-2")
		waitForOutputEquals(oc, project1, "route/unsrv-3", "{.metadata.name}", "unsrv-3")
		waitForOutputEquals(oc, project1, "route/unsrv-4", "{.metadata.name}", "unsrv-4")

		compat_otp.By("Add labels to 3 routes")
		...
		compat_otp.By("Patch the custom ingress-controllers with In matchExpressions routeSelector")
		routerpod := getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)
		...
		compat_otp.By("Check if route unsrv-1 and unsrv-2 are admitted by the custom IC with In matchExpressions routeSelector, while route unsrv-3 and unsrv-4 not")
		jsonPath := "{.status.ingress[?(@.routerName==\"" + ingctrl.name + "\")].conditions[?(@.type==\"Admitted\")].status}"
		ensureRouteIsAdmittedByIngressController(oc, project1, "unsrv-1", ingctrl.name)
		ensureRouteIsAdmittedByIngressController(oc, project1, "unsrv-2", ingctrl.name)
		waitForOutputEquals(oc, project1, "route/unsrv-3", jsonPath, "")
		waitForOutputEquals(oc, project1, "route/unsrv-4", jsonPath, "")

		compat_otp.By("Patch the custom ingress-controllers with NotIn matchExpressions routeSelector")
		routerpod = getOneRouterPodNameByIC(oc, ingctrl.name)
		...
		compat_otp.By("Check if route unsrv-3 and unsrv-4 are admitted by the custom IC with NotIn matchExpressions routeSelector, while route unsrv-1 and unsrv-2 not")
		ensureRouteIsAdmittedByIngressController(oc, project1, "unsrv-3", ingctrl.name)
		waitForOutputEquals(oc, project1, "route/unsrv-1", jsonPath, "")
		waitForOutputEquals(oc, project1, "route/unsrv-2", jsonPath, "")
		ensureRouteIsAdmittedByIngressController(oc, project1, "unsrv-4", ingctrl.name)

		compat_otp.By("Patch the custom ingress-controllers with Exists matchExpressions routeSelector")
		routerpod = getOneRouterPodNameByIC(oc, ingctrl.name)
		...
		compat_otp.By("Check if route unsrv-1, unsrv-2 and unsrv-3 are admitted by the custom IC with Exists matchExpressions routeSelector, while route unsrv-4 not")
		ensureRouteIsAdmittedByIngressController(oc, project1, "unsrv-1", ingctrl.name)
		ensureRouteIsAdmittedByIngressController(oc, project1, "unsrv-2", ingctrl.name)
		ensureRouteIsAdmittedByIngressController(oc, project1, "unsrv-3", ingctrl.name)
		waitForOutputEquals(oc, project1, "route/unsrv-4", jsonPath, "")

		compat_otp.By("Patch the custom ingress-controllers with DoesNotExist matchExpressions routeSelector")
		routerpod = getOneRouterPodNameByIC(oc, ingctrl.name)
		...
		compat_otp.By("Check if route unsrv-4 is admitted by the custom IC with DoesNotExist matchExpressions routeSelector, while route unsrv-1, unsrv-2 and unsrv-3 not")
		ensureRouteIsAdmittedByIngressController(oc, project1, "unsrv-4", ingctrl.name)
		waitForOutputEquals(oc, project1, "route/unsrv-1", jsonPath, "")
		waitForOutputEquals(oc, project1, "route/unsrv-2", jsonPath, "")
		waitForOutputEquals(oc, project1, "route/unsrv-3", jsonPath, "")
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-NonPreRelease-Medium-60013-matchExpressions for namespaceSelector defined in an ingress-controller", func() {
		...
		compat_otp.By("create 3 more projects")
		project1 := oc.Namespace()
		oc.SetupProject()
		...
		compat_otp.By("Create an unsecure service and its backend pod, create the route in each of the 4 projects, then wait for some time until the backend pod and route are available")
		...
		compat_otp.By("Add labels to 3 projects")
		...
		compat_otp.By("Patch the custom ingresscontroller with In matchExpressions namespaceSelector")
		routerpod := getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)
		...
		compat_otp.By("Check if route shard-ns1 and shard-ns2 are admitted by the custom IC with In matchExpressions namespaceSelector, while route shard-ns3 and shard-ns4 not")
		jsonPath := "{.status.ingress[?(@.routerName==\"" + ingctrl.name + "\")].conditions[?(@.type==\"Admitted\")].status}"
		...
		compat_otp.By("Patch the custom ingresscontroller with NotIn matchExpressions namespaceSelector")
		routerpod = getOneRouterPodNameByIC(oc, ingctrl.name)
		...
		compat_otp.By("Check if route shard-ns3 and shard-ns4 are admitted by the custom IC with NotIn matchExpressions namespaceSelector, while route shard-ns1 and shard-ns2 not")
		...
		compat_otp.By("Patch the custom ingresscontroller with Exists matchExpressions namespaceSelector")
		routerpod = getOneRouterPodNameByIC(oc, ingctrl.name)
		...
		compat_otp.By("Check if route shard-ns1, shard-ns2 and shard-ns3 are admitted by the custom IC with Exists matchExpressions namespaceSelector, while route shard-ns4 not")
		ensureRouteIsAdmittedByIngressController(oc, project1, "shard-ns1", ingctrl.name)
		ensureRouteIsAdmittedByIngressController(oc, project2, "shard-ns2", ingctrl.name)
		ensureRouteIsAdmittedByIngressController(oc, project3, "shard-ns3", ingctrl.name)
		waitForOutputEquals(oc, project4, "route/shard-ns4", jsonPath, "")

		compat_otp.By("Patch the custom ingresscontroller with DoesNotExist matchExpressions namespaceSelector")
		routerpod = getOneRouterPodNameByIC(oc, ingctrl.name)
		...
		compat_otp.By("Check if route shard-ns4 is admitted by the custom IC with DoesNotExist matchExpressions namespaceSelector, while route shard-ns1, shard-ns2 and shard-ns3 not")
		ensureRouteIsAdmittedByIngressController(oc, project4, "shard-ns4", ingctrl.name)
		waitForOutputEquals(oc, project1, "route/shard-ns1", jsonPath, "")
		waitForOutputEquals(oc, project2, "route/shard-ns2", jsonPath, "")
		waitForOutputEquals(oc, project3, "route/shard-ns3", jsonPath, "")
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-Critical-62530-openshift ingress operator is failing to update router-certs [Serial]", func() {
		...
		compat_otp.By("Create a tls secret in openshift-ingress ns")
		...
		compat_otp.By("Patch defaultCertificate with custom secret to the IC")
		...
		compat_otp.By("Check the router-certs in the openshift-config-managed namespace, the data is 1, which should not be increased")
	...
	g.It("Author:asood-NonHyperShiftHOST-ConnectedOnly-ROSA-OSD_CCS-Medium-63832-Cluster ingress health checks and routes fail on swapping application router between public and private", func() {
		...
		compat_otp.By("1. Annotate ingress controller")
		...
		compat_otp.By("2. Get the health check node port")
		...
		for i := 0; i < len(strategyScope); i++ {
			compat_otp.By("3. Change the endpoint publishing strategy")
			...
			compat_otp.By("3.1 Check the state of custom ingress operator")
			ensureCustomIngressControllerAvailable(oc, ingctrl.name)

			compat_otp.By("3.2 Check the pods are in running state")
			podList, podListErr := compat_otp.GetAllPodsWithLabel(oc, namespace, "ingresscontroller.operator.openshift.io/deployment-ingresscontroller="+ingctrl.name)
			o.Expect(podListErr).NotTo(o.HaveOccurred())
			o.Expect(len(podList)).ShouldNot(o.Equal(0))
			podName := podList[0]

			compat_otp.By("3.3 Get node name of one of the pod")
			nodeName, nodeNameErr := compat_otp.GetPodNodeName(oc, namespace, podName)
			o.Expect(nodeNameErr).NotTo(o.HaveOccurred())

			compat_otp.By("3.4. Get new health check node port")
			...
			compat_otp.AssertWaitPollNoErr(err, fmt.Sprintf("Failed to get health check node port %s", err))

			compat_otp.By("3.5. Check endpoint is 1")
	...
	g.It("Author:mjoseph-NonHyperShiftHOST-Critical-64611-Ingress operator support for private hosted zones in Shared VPC clusters", func() {
		...
		compat_otp.By("Pre-flight check for the shared VPC platform")
		...
		compat_otp.By("1. Check the STS Role in the cluster")
		...
		compat_otp.By("2. Check whether the privateZoneIAMRole is created using the ARN")
		...
		compat_otp.By("3. Check the default DNS management status")
		...
		compat_otp.By("4. Collecting the public zone and private zone id from dns config")
		...
		compat_otp.By("5. Collecting zone details from default ingress controller and cross checking it wih dns config details")
		checkDnsRecordsInIngressOperator(oc, "default-wildcard", privateZoneId, publicZoneId)

		compat_otp.By("6. Check the default dnsrecord of the ingress operator to confirm there is no degardes")
		...
		compat_otp.By("7. Create two custom ingresscontrollers")
		baseDomain := getBaseDomain(oc)
		ingctrl1.domain = ingctrl1.name + "." + baseDomain
		...
		compat_otp.By("8. Check the custom DNS management status")
		...
		compat_otp.By("9. Collecting zone details from custom ingress controller and cross checking it wih dns zone details")
		...
		compat_otp.By("10. Check the custom dnsrecord of the ingress operator to confirm there is no degardes")
	...
	g.It("Author:mjoseph-High-65827-allow Ingress to modify the HAProxy Log Length when using a Sidecar", func() {
		...
		compat_otp.By("2. Check the env variable of the router pod to verify the default log length")
		newrouterpod := getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)
		logLength := readRouterPodEnv(oc, newrouterpod, "ROUTER_LOG_MAX_LENGTH")
		o.Expect(logLength).To(o.ContainSubstring(`ROUTER_LOG_MAX_LENGTH=1024`))

		compat_otp.By("3. Check the haproxy config on the router pod to verify the default log length is enabled")
		checkoutput := readRouterPodData(oc, newrouterpod, "cat haproxy.config", "1024")
		o.Expect(checkoutput).To(o.ContainSubstring(`log /var/lib/rsyslog/rsyslog.sock len 1024 local1 info`))

		compat_otp.By("4. Patch the existing custom ingress controller with minimum log length value")
		...
		compat_otp.By("5. Check the env variable of the router pod to verify the minimum log length")
		newrouterpod = getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)
		minimumlogLength := readRouterPodEnv(oc, newrouterpod, "ROUTER_LOG_MAX_LENGTH")
		o.Expect(minimumlogLength).To(o.ContainSubstring(`ROUTER_LOG_MAX_LENGTH=480`))

		compat_otp.By("6. Patch the existing custom ingress controller with maximum log length value")
		...
		compat_otp.By("7. Check the env variable of the router pod to verify the maximum log length")
		newrouterpod = getOneNewRouterPodFromRollingUpdate(oc, ingctrl.name)
		maximumlogLength := readRouterPodEnv(oc, newrouterpod, "ROUTER_LOG_MAX_LENGTH")
	...
	g.It("Author:mjoseph-Low-65903-ingresscontroller should deny invalid maxlengh value when using a Sidecar", func() {
		...
		compat_otp.By("2. Check the haproxy config on the syslog router pod to verify the default log length")
		...
		compat_otp.By("3. Check the haproxy config on the sidecar router pod to verify the default log length")
		...
		compat_otp.By("4. Patch the existing sidecar ingress controller with log length value less than minimum threshold")
		...
		compat_otp.By("5. Patch the existing sidecar ingress controller with log length value more than maximum threshold")
	...
	g.It("Author:shudili-NonHyperShiftHOST-ROSA-OSD_CCS-ARO-High-75907-Ingress Operator should not always remain in the progressing state [Disruptive]", func() {
		...
		compat_otp.By("1. Create a configmap with empty configration")
		...
		compat_otp.By("2. Create a ingresscontroller for OCPBUGS-35027")
		baseDomain := getBaseDomain(oc)
		...
		compat_otp.By("3. Check the custom router pod should not be created for the custom ingresscontroller was abnormal")
		...
		compat_otp.By("4. Delete the custom ingress controller, and then check the logs that clientca-configmap finalizer log should not appear")
		...
		compat_otp.By("5. Create the custom ingress controller for OCPBUGS-35454")
		...
		compat_otp.By(`6. Check the service's spec ports of http/https/metrics`)
		...
		compat_otp.By(`7. Check the service's ep of http/https/metrics`)
		...
		compat_otp.By(`8. Check the configuration update for the custom router deployment and the internal service`)
		...
		compat_otp.By(`9. Check the service's sessionAffinity, which should be None`)
		...
		compat_otp.By(`10. Patch the custom ingress controller with other http/https/metrics ports`)
		...
		compat_otp.By(`11. Check the service's ep of http/https/metrics, which should be updated to the specified ports`)
		output = getByJsonPath(oc, "openshift-ingress", "ep/router-internal-"+ingctrlhp35454.name, "{.subsets[0].ports}")
		o.Expect(output).Should(o.And(
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-High-75908-http2 connection coalescing component routing should not be broken with single certificate [Disruptive]", func() {
		...
		compat_otp.By("1.0: skip for http2 not enabled clusters")
		routerpod := getOneRouterPodNameByIC(oc, "default")
		...
		compat_otp.By("2.0: Get some info including hostnames of console/oauth route for the testing")
		...
		compat_otp.By("3.0: Use openssl to create the certification and key")
		defer os.RemoveAll(dirname)
		err := os.MkdirAll(dirname, 0755)
		o.Expect(err).NotTo(o.HaveOccurred())

		compat_otp.By("3.1: Create a new self-signed CA including the ca certification and ca key")
		opensslCmd := fmt.Sprintf(`openssl req -x509 -newkey rsa:2048 -days %d -keyout %s -out %s -nodes -subj %s`, validity, caKey, caCrt, caSubj)
		_, err = exec.Command("bash", "-c", opensslCmd).Output()
		o.Expect(err).NotTo(o.HaveOccurred())

		compat_otp.By("3.2: Create the user CSR and the user key")
		opensslCmd = fmt.Sprintf(`openssl req -newkey rsa:2048 -nodes -keyout %s  -out %s -subj "/CN=*.%s"`, usrKey, usrCsr, appsDomain)
		_, err = exec.Command("bash", "-c", opensslCmd).Output()
		o.Expect(err).NotTo(o.HaveOccurred())

		compat_otp.By("3.3: Create the user certification")
		...
		compat_otp.By("4.0: Create the custom secret on the cluster with the created user certification and user key")
		...
		compat_otp.By("5.0: path the ingress with the console route host and the custom secret")
...
`, consoleRoute)
		...
		compat_otp.By("6.0: Check the console route has HTTP/2 enabled")
		...
		compat_otp.By("7.0: Check console certificate has different SHA1 Fingerprint with OAuth certificate and default certificate, by using openssl command")
	...
	g.It("Author:shudili-NonHyperShiftHOST-ROSA-OSD_CCS-ARO-High-75909-Ingress Operator should not always remain in the progressing state [Disruptive]", func() {
		...
		compat_otp.By("1. after the cluster is ready, check openshift-ingress-operator logs, which should not contain updated internal service")
		...
		compat_otp.By("2. Create two custom ingresscontrollers for OCPBUGS-34757")
		baseDomain := getBaseDomain(oc)
		...
		compat_otp.By("3. Check the logs again after the custom ingresscontrollers are ready, which should not contain updated internal service")
		output, err = oc.AsAdmin().WithoutNamespace().Run("logs").Args("-n", "openshift-ingress-operator", "deployment/ingress-operator").Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(strings.Contains(output, "updated internal service")).NotTo(o.BeTrue())
		...
		// OCPBUGS-34110
		compat_otp.By("4. delete the ingress-operator pod")
		...
		compat_otp.By("5. Create two custom ingresscontrollers for OCPBUGS-34888")
		...
		compat_otp.By("6. Check there was not the updated router deployment log")
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-Critical-77283-Router should support SHA1 CA certificates in the default certificate chain", func() {
		...
		compat_otp.By("1.0: Use openssl to create the certification and key")
		defer os.RemoveAll(dirname)
		err := os.MkdirAll(dirname, 0755)
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		compat_otp.By("1.1: Create a new self-signed sha1 root CA including the ca certification and ca key")
		...
		compat_otp.By("1.2: Create the user CSR and the user key")
		opensslCmd = fmt.Sprintf(`openssl req -newkey rsa:2048 -nodes -keyout %s  -out %s -subj %s`, usrKey, usrCsr, usrSubj)
		_, err = exec.Command("bash", "-c", opensslCmd).Output()
		o.Expect(err).NotTo(o.HaveOccurred())

		compat_otp.By("1.3: Create the extension file, then create the user certification")
		...
		compat_otp.By("1.4: create the file including the sha1 certification and user certification")
		catCmd := fmt.Sprintf(`cat %s %s > %s`, usrCrt, caCrt, combinationCrt)
		_, err = exec.Command("bash", "-c", catCmd).Output()
		o.Expect(err).NotTo(o.HaveOccurred())

		compat_otp.By("2.0: Create the custom secret on the cluster with the created the combination certifications and user key")
		...
		compat_otp.By("3.0: Create the custom ingresscontroller for the testing")
...
`)
		...
		compat_otp.By("4.0: Check the ingress co, it should be upgradable")
		...
		compat_otp.By("5.0: The canary route is accessable")
