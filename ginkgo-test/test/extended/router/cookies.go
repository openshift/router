...
var _ = g.Describe("[sig-network-edge] Network_Edge Component_Router", func() {
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-Critical-11903-haproxy cookies based sticky session for unsecure routes", func() {
		var (
			...
			cookie              = "/tmp/OCP-11903-cookie"
		)

		compat_otp.By("1.0: Prepare file for testing")
		...
		compat_otp.By("4.0: Curl the http route, make sure the second server is hit")
		...
		compat_otp.By("5.0: Curl the http route again, make sure the first server is hit")
		curlCmd = fmt.Sprintf(`curl http://%s -s --connect-timeout 10`, routehost)
		expectOutput = []string{"Hello-OpenShift " + srvPodList[0] + " http-8080"}
		repeatCmdOnClient(oc, curlCmd, expectOutput, 60, 1)

		compat_otp.By("6.0: Curl the http route with the specified cookie for 10 times, expect all are forwarded to the second server")
		...
		// OCP-11679
		compat_otp.By(`7.0: Disable haproxy hash based sticky session for the route by adding 'disable cookies' annotation to it`)
		setAnnotation(oc, ns, "route/"+unsecSvcName, "haproxy.router.openshift.io/disable_cookies=true")

		compat_otp.By("8.0: Check the disable cookies configuration in haproxy")
		...
		compat_otp.By("9.0: Curl the http route with the specified cookie for 10 times again, expect the requests are forwarded to the two servers")
		_, result = repeatCmdOnClient(oc, curlCmd, expectOutput, 120, 10)
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-Critical-15872-can set cookie name for unsecure routes by annotation", func() {
		...
		routerPod := getOneRouterPodNameByIC(oc, "default")
		ensureHaproxyBlockConfigMatchRegexp(oc, routerPod, "be_http:"+ns+":"+unsecSvcName, []string{"unsecure-cookie_1"})
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-Critical-35547-router.openshift.io/cookie-same-site route annotation accepts None Lax or Strict attribute for edge routes", func() {
		...
		compat_otp.By("1.0: Deploy a project with two server pods and the service")
		...
		ensureRouteIsAdmittedByIngressController(oc, project1, "edge35547", "default")
		...
		routerPod := getOneRouterPodNameByIC(oc, "default")
		ensureHaproxyBlockConfigContains(oc, routerPod, "be_edge_http:"+project1+":edge35547", []string{"SameSite=Strict"})
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-Critical-35548-router.openshift.io/cookie-same-site route annotation accepts None Lax or Strict attribute for Reencrypt routes", func() {
		...
		ensureRouteIsAdmittedByIngressController(oc, project1, "reen35548", "default")
		...
		routerPod := getOneRouterPodNameByIC(oc, "default")
		ensureHaproxyBlockConfigContains(oc, routerPod, "be_secure:"+project1+":reen35548", []string{"SameSite=Lax"})
		...
		ensureHaproxyBlockConfigContains(oc, routerPod, "be_secure:"+project1+":reen35548", []string{"SameSite=Strict"})
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-Low-35549-router.openshift.io/cookie-same-site route annotation does not work with Passthrough routes", func() {
		...
		ensureRouteIsAdmittedByIngressController(oc, project1, "pass35549", "default")
