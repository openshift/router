...
var _ = g.Describe("[sig-network-edge] Network_Edge Component_Router", func() {
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-High-16870-No health check when there is only one endpoint for a route", func() {
		...
		backendConfig = ensureHaproxyBlockConfigContains(oc, routerpod, "be_http:"+ns+":"+unSecSvcName, []string{"check inter"})
	...
	g.It("Author:mjoseph-ROSA-OSD_CCS-ARO-High-16872-Health check when there are multi service and each service has one backend", func() {
		...
		o.Expect(backendConfig).To(o.MatchRegexp("server pod:" + webServerDeploy1.deployName + ".+check inter 5000ms"))
		o.Expect(backendConfig).To(o.MatchRegexp("server pod:" + webServerDeploy2.deployName + ".+check inter 5000ms"))
	})
...
`, cmName)
		...
		compat_otp.By("5.0 Deploy a project with a deployment and a client pod")
		...
		err = oc.AsAdmin().WithoutNamespace().Run("cp").Args("-n", project1, dirname, project1+"/"+clientPodName+":"+podDirname).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())

		compat_otp.By("6.0 Create a reencrypt route and a passthrough route inside the project")
		...
		ensureRouteIsAdmittedByIngressController(oc, project1, "route-pass", "default")
		...
		curlCmd := []string{"-n", project1, clientPodName, "--", "curl", "https://" + routehost, "-sI", "--cacert", podCaCrt, "--cert", podUsrCrt, "--key", podUsrKey, "--http2", "--resolve", toDst, "--connect-timeout", "10"}
...
`)
		...
		compat_otp.By("2.0 Deploy a project with a deployment and a client pod")
		project1 := oc.Namespace()
		createResourceFromFile(oc, project1, updatedDeployFile)
		waitForOutputEquals(oc, project1, "deployment/web-server-deploy", "{.status.replicas}", "1", 180*time.Second)
		serverPod := getPodListByLabel(oc, project1, "name=web-server-deploy")[0]
		waitForOutputEquals(oc, project1, "pod/"+serverPod, "{.status.phase}", "Running")
		waitForOutputEquals(oc, project1, "pod/"+serverPod, "{.status.conditions[?(@.type==\"Ready\")].status}", "False")
		...
		backendConfig := ensureHaproxyBlockConfigContains(oc, routerpod, "be_http:"+project1+":"+unSecSvcName, []string{unSecSvcName})
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		waitForOutputEquals(oc, project1, "pod/"+serverPod, "{.status.conditions[?(@.type==\"Ready\")].status}", "True")

		compat_otp.By("7.0 Check haproxy.config which should have the server slot")
		ensureHaproxyBlockConfigContains(oc, routerpod, "be_http:"+project1+":"+unSecSvcName, []string{serverSlot})
	...
	g.It("Author:aiyengar-ROSA-OSD_CCS-ARO-LEVEL0-High-41042-The Power-of-two balancing features defaults to random LB algorithm instead of leastconn for REEN/Edge/insecure routes", func() {
		...
		compat_otp.By("Expose a edge/insecure/REEN/passthrough type routes via the services inside project")
	...
	g.It("Author:aiyengar-High-41187-The Power of two balancing  honours the per route balancing algorithm defined via haproxy.router.openshift.io/balance annotation", func() {
		...
		compat_otp.By("Expose a route from the project and set route LB annotation")
		createRoute(oc, project1, "http", unsecsvcName, unsecsvcName, []string{})
		output, err := oc.Run("get").Args("route").Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(output).To(o.ContainSubstring(unsecsvcName))
	...
	g.It("Author:shudili-Medium-42940-User can customize HAProxy 2.0 Error Page", func() {
		...
		compat_otp.By("Expose an route with the unsecure service inside the project")
		routehost := srvName + "-" + project1 + "." + ingctrl.domain
		srvErr := oc.Run("expose").Args("service", srvName, "--hostname="+routehost).Execute()
		o.Expect(srvErr).NotTo(o.HaveOccurred())
		waitForOutputEquals(oc, project1, "route", "{.items[0].metadata.name}", srvName)
		...
		compat_otp.AssertWaitPollNoErr(err, fmt.Sprintf("resource %!v(MISSING) does not disapper", "pod/"+podname))
	...
	g.It("Author:jechen-High-43115-Configmap mounted on router volume after ingresscontroller has spec field HttpErrorCodePage populated with configmap name", func() {
		...
		compat_otp.AssertWaitPollNoErr(err, fmt.Sprintf("cm %!v(MISSING) not found", configmapName))
		...
		compat_otp.AssertWaitPollNoErr(err, fmt.Sprintf("cm %!v(MISSING) not found", expectedCmName))
	...
	g.It("Author:hongli-LEVEL0-Critical-47344-check haproxy router v4v6 mode", func() {
		...
		e2e.Logf("the cluster IP stack type is: %!v(MISSING)", ipStackType)
	...
	g.It("Author:aiyengar-ROSA-OSD_CCS-ARO-High-52738-The Power-of-two balancing features switches to source algorithm for passthrough routes", func() {
		...
		compat_otp.By("Expose a passthrough type routes via the services inside project")
		passthRoute := "route-passth" + "-" + project1 + "." + baseDomain
	...
	g.It("Author:shudili-High-55825-the 503 Error page should not contain license for a vulnerable release of Bootstrap", func() {
		...
		compat_otp.By("Deploy a project with a client pod used to send traffic")
		project1 := oc.Namespace()
		compat_otp.By("create a client pod")
		createResourceFromFile(oc, project1, clientPod)
		ensurePodWithLabelReady(oc, project1, clientPodLabel)
	...
	g.It("Author:shudili-High-57001-edge terminated h2 (gRPC) connections need a haproxy template change to work correctly", func() {
		...
		compat_otp.By("Create an edge route with the h2c service inside the project")
	...
	g.It("Author:mjoseph-ROSA-OSD_CCS-ARO-High-77862-Check whether required ENV varibales are configured after enabling Dynamic Configuration Manager", func() {
		...
		compat_otp.By("3. Expose a route with the unsecure service inside the project")
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-High-77892-Dynamic Configuration Manager for plain HTTP route [Serial]", func() {
		...
		compat_otp.By("2.0 Deploy a project with a deployment, a HTTP route and a client pod")
		project1 := oc.Namespace()
		createResourceFromFile(oc, project1, testPodSvc)
		ensurePodWithLabelReady(oc, project1, "name="+srvdmInfo)
		createRoute(oc, project1, "http", "unsecure77892", svcName, []string{"--hostname=" + routehost})
		ensureRouteIsAdmittedByIngressController(oc, project1, "unsecure77892", "default")
		createResourceFromFile(oc, project1, clientPod)
		ensurePodWithLabelReady(oc, project1, clientPodLabel)
		...
		for i := 1; i < desiredReplicas; i++ {
			...
			waitForOutputEquals(oc, project1, "deployment/"+srvdmInfo, "{.status.availableReplicas}", strconv.Itoa(i+1))
...
`, cmName)
		...
		compat_otp.By("6.0 Create a reencrypt route inside the project")
		createRoute(oc, project1, "reencrypt", "route-reen", svcName, []string{"--hostname=" + routehost, "--ca-cert=" + caCrt, "--cert=" + usrCrt, "--key=" + usrKey})
		ensureRouteIsAdmittedByIngressController(oc, project1, "route-reen", "default")
		...
		curlCmd = []string{"-n", project1, clientPodName, "--", "curl", "https://" + routehost, "-sI", "--cacert", podCaCrt, "--cert", podUsrCrt, "--key", podUsrKey, "--http1.1", "--resolve", toDst, "--connect-timeout", "10"}
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-High-77973-Dynamic Configuration Manager for edge route [Serial]", func() {
		...
		compat_otp.By("2.0 Deploy a project with a deployment, an edge route and a client pod")
		project1 := oc.Namespace()
		createResourceFromFile(oc, project1, testPodSvc)
		ensurePodWithLabelReady(oc, project1, "name="+srvdmInfo)
		createRoute(oc, project1, "edge", "edge77973", svcName, []string{"--hostname=" + routehost})
		ensureRouteIsAdmittedByIngressController(oc, project1, "edge77973", "default")
		createResourceFromFile(oc, project1, clientPod)
		ensurePodWithLabelReady(oc, project1, clientPodLabel)
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-High-77974-Dynamic Configuration Manager for passthrough route [Serial]", func() {
		...
		compat_otp.By("2.0 Deploy a project with a deployment, a passthrough route and a client pod")
		project1 := oc.Namespace()
		createResourceFromFile(oc, project1, testPodSvc)
		ensurePodWithLabelReady(oc, project1, "name="+srvdmInfo)
		createRoute(oc, project1, "passthrough", "passth77974", svcName, []string{"--hostname=" + routehost})
		ensureRouteIsAdmittedByIngressController(oc, project1, "passth77974", "default")
		createResourceFromFile(oc, project1, clientPod)
		ensurePodWithLabelReady(oc, project1, clientPodLabel)
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-High-77975-Dynamic Configuration Manager for reencrypt route [Serial]", func() {
		...
		compat_otp.By("2.0 Deploy a project with a deployment, a reencrypt route and a client pod")
		project1 := oc.Namespace()
		createResourceFromFile(oc, project1, testPodSvc)
		ensurePodWithLabelReady(oc, project1, "name="+srvdmInfo)
		createRoute(oc, project1, "reencrypt", "reen77975", svcName, []string{"--hostname=" + routehost})
		ensureRouteIsAdmittedByIngressController(oc, project1, "reen77975", "default")
		createResourceFromFile(oc, project1, clientPod)
		ensurePodWithLabelReady(oc, project1, clientPodLabel)
	...
	g.It("Author:shudili-ROSA-OSD_CCS-ARO-Medium-78239-traffic test for dynamic servers", func() {
		...
		ensureRouteIsAdmittedByIngressController(oc, project1, "unsecure78239", "default")
		ensureRouteIsAdmittedByIngressController(oc, project1, "reen78239", "default")
		...
		waitForOutputEquals(oc, project1, "deployment/"+srvdmInfo, "{.status.availableReplicas}", strconv.Itoa(desiredReplicas))
