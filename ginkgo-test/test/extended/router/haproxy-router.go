...
var _ = g.Describe("[sig-network-edge] Network_Edge Component_Router", func() {
	...
	g.It("Author:mjoseph-ROSA-OSD_CCS-ARO-Critical-15044-The backend health check interval of unsecure/edge/passthrough/reencrypt route can be set by annotation", func() {
		...
		httpSearchOutput := readHaproxyConfig(oc, defaultContPod, ns+":"+"route-http", "-A15", ns)
		o.Expect(httpSearchOutput).To(o.ContainSubstring("check inter 200ms"))
		edgeSearchOutput := readHaproxyConfig(oc, defaultContPod, ns+":"+"route-edge", "-A15", ns)
		o.Expect(edgeSearchOutput).To(o.ContainSubstring("check inter 300ms"))
		passSearchOutput := readHaproxyConfig(oc, defaultContPod, ns+":"+"route-pass", "-A10", ns)
		o.Expect(passSearchOutput).To(o.ContainSubstring("check inter 400ms"))
		reenSearchOutput := readHaproxyConfig(oc, defaultContPod, ns+":"+"route-reen", "-A15", ns)
		o.Expect(reenSearchOutput).To(o.ContainSubstring("check inter 500ms"))
	})
	...
	g.It("Author:mjoseph-ROSA-OSD_CCS-ARO-High-16872-Health check when there are multi service and each service has one backend", func() {
		...
		httpSearchOutput := readHaproxyConfig(oc, routerPod, ns+":"+unsecureRouteName, "-A20", ns)
		...
		backendConfig := readHaproxyConfig(oc, routerPod, backendBegin, "-A20", "pod:"+webServerDeploy2.deployName)
...
`)
		...
		waitForOutput(oc, project1, "deployment/web-server-deploy", "{.status.replicas}", "1")
	...
	g.It("Author:shudili-Medium-42940-User can customize HAProxy 2.0 Error Page", func() {
		...
		exutil.AssertWaitPollNoErr(err, fmt.Sprintf("resource %!v(MISSING) does not disapper", "pod/"+podname))
	...
	g.It("Author:jechen-High-43115-Configmap mounted on router volume after ingresscontroller has spec field HttpErrorCodePage populated with configmap name", func() {
		...
		exutil.AssertWaitPollNoErr(err, fmt.Sprintf("cm %!v(MISSING) not found", configmapName))
		...
		exutil.AssertWaitPollNoErr(err, fmt.Sprintf("cm %!v(MISSING) not found", expectedCmName))
