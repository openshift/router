...
The `local-router.sh` script configures the environment to satisfy some pre-requisites and runs the router.
Feel free to analyze it and run commands manually instead.
...
> Run `./hack/local-router.sh prepare` before the very first run, and whenever a clean environment is needed.
