...
// It also updates the state of the enpoint to `drain` or `ready` if the weight going to, or coming from `0`.
...
// because of that it is important to not replace servers in case of single replica, this would cause an outage
// until HAProxy is reloaded.
