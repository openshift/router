...
func ManageCRLs(caBundleFilename string, caUpdateChannel <-chan struct{}, updateCallback func(bool)) {
	go func() {
		...
		nextUpdate := time.Now()
		updated := false
		shouldHaveCRLs := false
		for {
			// All of the functions that could return an error also return data that needs to persist between iterations
			// of the for loop, so we need to use var instead of := to declare err.
			var err error
			if caUpdated {
				...
				if err != nil {
					...
					// TODO: Should we back off if we can't parse the CA bundle, or just wait until the CA bundle is updated again?
